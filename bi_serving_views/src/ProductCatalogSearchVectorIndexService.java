...
/**
 * Enterprise Service Processor: ProductCatalogSearchVectorIndexService.
 * Maintains dense HNSW vector index of product embeddings for semantic visual search.
 */
public class ProductCatalogSearchVectorIndexService {

    private static final Logger LOGGER = Logger.getLogger(ProductCatalogSearchVectorIndexService.class.getName());
    private final Map<String, ProductCatalogSearchVectorIndexModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ProductCatalogSearchVectorIndexService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ProductCatalogSearchVectorIndexModel model = new ProductCatalogSearchVectorIndexModel(
                mutId,
    ...
    public ProductCatalogSearchVectorIndexModel processIncomingMutation(ProductCatalogSearchVectorIndexModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ProductCatalogSearchVectorIndexService");
    ...
    public Optional<ProductCatalogSearchVectorIndexModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ProductCatalogSearchVectorIndexModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ProductCatalogSearchVectorIndexModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ProductCatalogSearchVectorIndexModel model = storageCache.get(mutationId);
        if (model != null) {
