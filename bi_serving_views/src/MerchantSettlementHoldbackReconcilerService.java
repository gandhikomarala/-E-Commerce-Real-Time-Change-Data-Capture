package com.ecommerce.lakehouse.views;
...
/**
 * Enterprise Service Processor: MerchantSettlementHoldbackReconcilerService.
 * Calculates merchant dispute holdbacks and 14-day rolling reserve release schedules.
 */
public class MerchantSettlementHoldbackReconcilerService {

    private static final Logger LOGGER = Logger.getLogger(MerchantSettlementHoldbackReconcilerService.class.getName());
    private final Map<String, MerchantSettlementHoldbackReconcilerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public MerchantSettlementHoldbackReconcilerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            MerchantSettlementHoldbackReconcilerModel model = new MerchantSettlementHoldbackReconcilerModel(
                mutId,
    ...
    public MerchantSettlementHoldbackReconcilerModel processIncomingMutation(MerchantSettlementHoldbackReconcilerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in MerchantSettlementHoldbackReconcilerService");
    ...
    public Optional<MerchantSettlementHoldbackReconcilerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<MerchantSettlementHoldbackReconcilerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<MerchantSettlementHoldbackReconcilerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        MerchantSettlementHoldbackReconcilerModel model = storageCache.get(mutationId);
        if (model != null) {
