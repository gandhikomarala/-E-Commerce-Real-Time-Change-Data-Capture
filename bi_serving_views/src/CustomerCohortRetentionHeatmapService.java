...
/**
 * Enterprise Service Processor: CustomerCohortRetentionHeatmapService.
 * Generates weekly and monthly customer retention cohort heatmaps for growth analytics.
 */
public class CustomerCohortRetentionHeatmapService {

    private static final Logger LOGGER = Logger.getLogger(CustomerCohortRetentionHeatmapService.class.getName());
    private final Map<String, CustomerCohortRetentionHeatmapModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CustomerCohortRetentionHeatmapService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CustomerCohortRetentionHeatmapModel model = new CustomerCohortRetentionHeatmapModel(
                mutId,
    ...
    public CustomerCohortRetentionHeatmapModel processIncomingMutation(CustomerCohortRetentionHeatmapModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CustomerCohortRetentionHeatmapService");
    ...
    public Optional<CustomerCohortRetentionHeatmapModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CustomerCohortRetentionHeatmapModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CustomerCohortRetentionHeatmapModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CustomerCohortRetentionHeatmapModel model = storageCache.get(mutationId);
        if (model != null) {
