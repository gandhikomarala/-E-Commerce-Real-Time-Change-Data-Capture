...
/**
 * Enterprise Service Processor: RegionalSalesTaxLiabilityLedgerService.
 * Maintains accurate audit trails of collected state and municipal sales taxes.
 */
public class RegionalSalesTaxLiabilityLedgerService {

    private static final Logger LOGGER = Logger.getLogger(RegionalSalesTaxLiabilityLedgerService.class.getName());
    private final Map<String, RegionalSalesTaxLiabilityLedgerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public RegionalSalesTaxLiabilityLedgerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            RegionalSalesTaxLiabilityLedgerModel model = new RegionalSalesTaxLiabilityLedgerModel(
                mutId,
    ...
    public RegionalSalesTaxLiabilityLedgerModel processIncomingMutation(RegionalSalesTaxLiabilityLedgerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in RegionalSalesTaxLiabilityLedgerService");
    ...
    public Optional<RegionalSalesTaxLiabilityLedgerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<RegionalSalesTaxLiabilityLedgerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<RegionalSalesTaxLiabilityLedgerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        RegionalSalesTaxLiabilityLedgerModel model = storageCache.get(mutationId);
        if (model != null) {
