...
/**
 * Enterprise Service Processor: ExecutiveGmvSummaryViewAggregatorService.
 * Pre-computes sub-second materialized views for C-suite gross merchandise value dashboards.
 */
public class ExecutiveGmvSummaryViewAggregatorService {

    private static final Logger LOGGER = Logger.getLogger(ExecutiveGmvSummaryViewAggregatorService.class.getName());
    private final Map<String, ExecutiveGmvSummaryViewAggregatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ExecutiveGmvSummaryViewAggregatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ExecutiveGmvSummaryViewAggregatorModel model = new ExecutiveGmvSummaryViewAggregatorModel(
                mutId,
    ...
    public ExecutiveGmvSummaryViewAggregatorModel processIncomingMutation(ExecutiveGmvSummaryViewAggregatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ExecutiveGmvSummaryViewAggregatorService");
    ...
    public Optional<ExecutiveGmvSummaryViewAggregatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ExecutiveGmvSummaryViewAggregatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ExecutiveGmvSummaryViewAggregatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ExecutiveGmvSummaryViewAggregatorModel model = storageCache.get(mutationId);
        if (model != null) {
