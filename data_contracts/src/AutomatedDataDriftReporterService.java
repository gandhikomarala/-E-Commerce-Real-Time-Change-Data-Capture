...
/**
 * Enterprise Service Processor: AutomatedDataDriftReporterService.
 * Generates daily HTML and JSON reports summarizing column distributions and drift metrics.
 */
public class AutomatedDataDriftReporterService {

    private static final Logger LOGGER = Logger.getLogger(AutomatedDataDriftReporterService.class.getName());
    private final Map<String, AutomatedDataDriftReporterModel> storageCache = new ConcurrentHashMap<>();
    ...
    public AutomatedDataDriftReporterService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            AutomatedDataDriftReporterModel model = new AutomatedDataDriftReporterModel(
                mutId,
    ...
    public AutomatedDataDriftReporterModel processIncomingMutation(AutomatedDataDriftReporterModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in AutomatedDataDriftReporterService");
    ...
    public Optional<AutomatedDataDriftReporterModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<AutomatedDataDriftReporterModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<AutomatedDataDriftReporterModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        AutomatedDataDriftReporterModel model = storageCache.get(mutationId);
        if (model != null) {
