...
/**
 * Enterprise Service Processor: DataQualityScorecardCalculatorService.
 * Calculates composite 0-100 data quality scorecards across Bronze, Silver, and Gold tiers.
 */
public class DataQualityScorecardCalculatorService {

    private static final Logger LOGGER = Logger.getLogger(DataQualityScorecardCalculatorService.class.getName());
    private final Map<String, DataQualityScorecardCalculatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public DataQualityScorecardCalculatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            DataQualityScorecardCalculatorModel model = new DataQualityScorecardCalculatorModel(
                mutId,
    ...
    public DataQualityScorecardCalculatorModel processIncomingMutation(DataQualityScorecardCalculatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in DataQualityScorecardCalculatorService");
    ...
    public Optional<DataQualityScorecardCalculatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<DataQualityScorecardCalculatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<DataQualityScorecardCalculatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        DataQualityScorecardCalculatorModel model = storageCache.get(mutationId);
        if (model != null) {
