...
/**
 * Enterprise Service Processor: WarehouseBinLocationStockAuditorService.
 * Tracks itemized physical warehouse aisle/shelf/bin allocations across fulfillment centers.
 */
public class WarehouseBinLocationStockAuditorService {

    private static final Logger LOGGER = Logger.getLogger(WarehouseBinLocationStockAuditorService.class.getName());
    private final Map<String, WarehouseBinLocationStockAuditorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public WarehouseBinLocationStockAuditorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            WarehouseBinLocationStockAuditorModel model = new WarehouseBinLocationStockAuditorModel(
                mutId,
    ...
    public WarehouseBinLocationStockAuditorModel processIncomingMutation(WarehouseBinLocationStockAuditorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in WarehouseBinLocationStockAuditorService");
    ...
    public Optional<WarehouseBinLocationStockAuditorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<WarehouseBinLocationStockAuditorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<WarehouseBinLocationStockAuditorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        WarehouseBinLocationStockAuditorModel model = storageCache.get(mutationId);
        if (model != null) {
