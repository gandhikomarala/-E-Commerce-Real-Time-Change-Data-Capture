...
/**
 * Enterprise Service Processor: SafetyStockBufferCalculationEngineService.
 * Computes dynamic safety stock levels based on lead time volatility and Poisson demand.
 */
public class SafetyStockBufferCalculationEngineService {

    private static final Logger LOGGER = Logger.getLogger(SafetyStockBufferCalculationEngineService.class.getName());
    private final Map<String, SafetyStockBufferCalculationEngineModel> storageCache = new ConcurrentHashMap<>();
    ...
    public SafetyStockBufferCalculationEngineService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            SafetyStockBufferCalculationEngineModel model = new SafetyStockBufferCalculationEngineModel(
                mutId,
    ...
    public SafetyStockBufferCalculationEngineModel processIncomingMutation(SafetyStockBufferCalculationEngineModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in SafetyStockBufferCalculationEngineService");
    ...
    public Optional<SafetyStockBufferCalculationEngineModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<SafetyStockBufferCalculationEngineModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<SafetyStockBufferCalculationEngineModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        SafetyStockBufferCalculationEngineModel model = storageCache.get(mutationId);
        if (model != null) {
