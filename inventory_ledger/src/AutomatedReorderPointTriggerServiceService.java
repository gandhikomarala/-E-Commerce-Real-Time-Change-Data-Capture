...
/**
 * Enterprise Service Processor: AutomatedReorderPointTriggerServiceService.
 * Generates automated vendor purchase orders when inventory dips below dynamically calculated ROP.
 */
public class AutomatedReorderPointTriggerServiceService {

    private static final Logger LOGGER = Logger.getLogger(AutomatedReorderPointTriggerServiceService.class.getName());
    private final Map<String, AutomatedReorderPointTriggerServiceModel> storageCache = new ConcurrentHashMap<>();
    ...
    public AutomatedReorderPointTriggerServiceService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            AutomatedReorderPointTriggerServiceModel model = new AutomatedReorderPointTriggerServiceModel(
                mutId,
    ...
    public AutomatedReorderPointTriggerServiceModel processIncomingMutation(AutomatedReorderPointTriggerServiceModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in AutomatedReorderPointTriggerServiceService");
    ...
    public Optional<AutomatedReorderPointTriggerServiceModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<AutomatedReorderPointTriggerServiceModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<AutomatedReorderPointTriggerServiceModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        AutomatedReorderPointTriggerServiceModel model = storageCache.get(mutationId);
        if (model != null) {
