package com.ecommerce.lakehouse.inventory;
...
/**
 * Enterprise Service Processor: SupplierPurchaseOrderReconciliationLedgerService.
 * Reconciles incoming supplier ASNs (Advance Shipping Notices) with warehouse dock scans.
 */
public class SupplierPurchaseOrderReconciliationLedgerService {

    private static final Logger LOGGER = Logger.getLogger(SupplierPurchaseOrderReconciliationLedgerService.class.getName());
    private final Map<String, SupplierPurchaseOrderReconciliationLedgerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public SupplierPurchaseOrderReconciliationLedgerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            SupplierPurchaseOrderReconciliationLedgerModel model = new SupplierPurchaseOrderReconciliationLedgerModel(
                mutId,
    ...
    public SupplierPurchaseOrderReconciliationLedgerModel processIncomingMutation(SupplierPurchaseOrderReconciliationLedgerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in SupplierPurchaseOrderReconciliationLedgerService");
    ...
    public Optional<SupplierPurchaseOrderReconciliationLedgerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<SupplierPurchaseOrderReconciliationLedgerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<SupplierPurchaseOrderReconciliationLedgerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        SupplierPurchaseOrderReconciliationLedgerModel model = storageCache.get(mutationId);
        if (model != null) {
