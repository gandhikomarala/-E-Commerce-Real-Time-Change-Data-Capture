...
/**
 * Enterprise Service Processor: MultiEchelonInventoryPositionOptimizerService.
 * Balances stock allocation across central fulfillment hubs and local micro-warehouses.
 */
public class MultiEchelonInventoryPositionOptimizerService {

    private static final Logger LOGGER = Logger.getLogger(MultiEchelonInventoryPositionOptimizerService.class.getName());
    private final Map<String, MultiEchelonInventoryPositionOptimizerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public MultiEchelonInventoryPositionOptimizerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            MultiEchelonInventoryPositionOptimizerModel model = new MultiEchelonInventoryPositionOptimizerModel(
                mutId,
    ...
    public MultiEchelonInventoryPositionOptimizerModel processIncomingMutation(MultiEchelonInventoryPositionOptimizerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in MultiEchelonInventoryPositionOptimizerService");
    ...
    public Optional<MultiEchelonInventoryPositionOptimizerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<MultiEchelonInventoryPositionOptimizerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<MultiEchelonInventoryPositionOptimizerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        MultiEchelonInventoryPositionOptimizerModel model = storageCache.get(mutationId);
        if (model != null) {
