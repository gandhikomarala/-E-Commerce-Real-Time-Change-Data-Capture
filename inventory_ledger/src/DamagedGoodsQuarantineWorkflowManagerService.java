...
/**
 * Enterprise Service Processor: DamagedGoodsQuarantineWorkflowManagerService.
 * Automates damaged stock inspection, vendor RMA return processing, and write-off logs.
 */
public class DamagedGoodsQuarantineWorkflowManagerService {

    private static final Logger LOGGER = Logger.getLogger(DamagedGoodsQuarantineWorkflowManagerService.class.getName());
    private final Map<String, DamagedGoodsQuarantineWorkflowManagerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public DamagedGoodsQuarantineWorkflowManagerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            DamagedGoodsQuarantineWorkflowManagerModel model = new DamagedGoodsQuarantineWorkflowManagerModel(
                mutId,
    ...
    public DamagedGoodsQuarantineWorkflowManagerModel processIncomingMutation(DamagedGoodsQuarantineWorkflowManagerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in DamagedGoodsQuarantineWorkflowManagerService");
    ...
    public Optional<DamagedGoodsQuarantineWorkflowManagerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<DamagedGoodsQuarantineWorkflowManagerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<DamagedGoodsQuarantineWorkflowManagerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        DamagedGoodsQuarantineWorkflowManagerModel model = storageCache.get(mutationId);
        if (model != null) {
