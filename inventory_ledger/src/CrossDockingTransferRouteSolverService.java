...
/**
 * Enterprise Service Processor: CrossDockingTransferRouteSolverService.
 * Solves optimal direct cross-docking vehicle transfer schedules without intermediate warehousing.
 */
public class CrossDockingTransferRouteSolverService {

    private static final Logger LOGGER = Logger.getLogger(CrossDockingTransferRouteSolverService.class.getName());
    private final Map<String, CrossDockingTransferRouteSolverModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CrossDockingTransferRouteSolverService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CrossDockingTransferRouteSolverModel model = new CrossDockingTransferRouteSolverModel(
                mutId,
    ...
    public CrossDockingTransferRouteSolverModel processIncomingMutation(CrossDockingTransferRouteSolverModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CrossDockingTransferRouteSolverService");
    ...
    public Optional<CrossDockingTransferRouteSolverModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CrossDockingTransferRouteSolverModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CrossDockingTransferRouteSolverModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CrossDockingTransferRouteSolverModel model = storageCache.get(mutationId);
        if (model != null) {
