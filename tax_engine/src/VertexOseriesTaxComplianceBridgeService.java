...
/**
 * Enterprise Service Processor: VertexOseriesTaxComplianceBridgeService.
 * Integrates with Vertex O Series tax engine for B2B tax exemption certificates.
 */
public class VertexOseriesTaxComplianceBridgeService {

    private static final Logger LOGGER = Logger.getLogger(VertexOseriesTaxComplianceBridgeService.class.getName());
    private final Map<String, VertexOseriesTaxComplianceBridgeModel> storageCache = new ConcurrentHashMap<>();
    ...
    public VertexOseriesTaxComplianceBridgeService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            VertexOseriesTaxComplianceBridgeModel model = new VertexOseriesTaxComplianceBridgeModel(
                mutId,
    ...
    public VertexOseriesTaxComplianceBridgeModel processIncomingMutation(VertexOseriesTaxComplianceBridgeModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in VertexOseriesTaxComplianceBridgeService");
    ...
    public Optional<VertexOseriesTaxComplianceBridgeModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<VertexOseriesTaxComplianceBridgeModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<VertexOseriesTaxComplianceBridgeModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        VertexOseriesTaxComplianceBridgeModel model = storageCache.get(mutationId);
        if (model != null) {
