...
/**
 * Enterprise Service Processor: EuVatOneStopShopOssLedgerService.
 * Maintains European Union VAT One-Stop Shop (OSS) quarterly tax return ledgers.
 */
public class EuVatOneStopShopOssLedgerService {

    private static final Logger LOGGER = Logger.getLogger(EuVatOneStopShopOssLedgerService.class.getName());
    private final Map<String, EuVatOneStopShopOssLedgerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public EuVatOneStopShopOssLedgerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            EuVatOneStopShopOssLedgerModel model = new EuVatOneStopShopOssLedgerModel(
                mutId,
    ...
    public EuVatOneStopShopOssLedgerModel processIncomingMutation(EuVatOneStopShopOssLedgerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in EuVatOneStopShopOssLedgerService");
    ...
    public Optional<EuVatOneStopShopOssLedgerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<EuVatOneStopShopOssLedgerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<EuVatOneStopShopOssLedgerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        EuVatOneStopShopOssLedgerModel model = storageCache.get(mutationId);
        if (model != null) {
