...
/**
 * Enterprise Service Processor: UsStateEconomicNexusThresholdAuditorService.
 * Monitors rolling 12-month sales volume and transaction count thresholds for state nexus.
 */
public class UsStateEconomicNexusThresholdAuditorService {

    private static final Logger LOGGER = Logger.getLogger(UsStateEconomicNexusThresholdAuditorService.class.getName());
    private final Map<String, UsStateEconomicNexusThresholdAuditorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public UsStateEconomicNexusThresholdAuditorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            UsStateEconomicNexusThresholdAuditorModel model = new UsStateEconomicNexusThresholdAuditorModel(
                mutId,
    ...
    public UsStateEconomicNexusThresholdAuditorModel processIncomingMutation(UsStateEconomicNexusThresholdAuditorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in UsStateEconomicNexusThresholdAuditorService");
    ...
    public Optional<UsStateEconomicNexusThresholdAuditorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<UsStateEconomicNexusThresholdAuditorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<UsStateEconomicNexusThresholdAuditorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        UsStateEconomicNexusThresholdAuditorModel model = storageCache.get(mutationId);
        if (model != null) {
