...
/**
 * Enterprise Service Processor: MerchantDisputeRiskFeatureAggregatorService.
 * Maintains merchant dispute rate and chargeback velocity features in Redis cluster.
 */
public class MerchantDisputeRiskFeatureAggregatorService {

    private static final Logger LOGGER = Logger.getLogger(MerchantDisputeRiskFeatureAggregatorService.class.getName());
    private final Map<String, MerchantDisputeRiskFeatureAggregatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public MerchantDisputeRiskFeatureAggregatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            MerchantDisputeRiskFeatureAggregatorModel model = new MerchantDisputeRiskFeatureAggregatorModel(
                mutId,
    ...
    public MerchantDisputeRiskFeatureAggregatorModel processIncomingMutation(MerchantDisputeRiskFeatureAggregatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in MerchantDisputeRiskFeatureAggregatorService");
    ...
    public Optional<MerchantDisputeRiskFeatureAggregatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<MerchantDisputeRiskFeatureAggregatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<MerchantDisputeRiskFeatureAggregatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        MerchantDisputeRiskFeatureAggregatorModel model = storageCache.get(mutationId);
        if (model != null) {
