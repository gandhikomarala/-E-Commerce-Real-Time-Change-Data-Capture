package com.ecommerce.lakehouse.features;
...
/**
 * Enterprise Service Processor: CustomerRealTimeCheckoutPropensityScorerService.
 * Computes real-time checkout propensity score based on in-session cart additions.
 */
public class CustomerRealTimeCheckoutPropensityScorerService {

    private static final Logger LOGGER = Logger.getLogger(CustomerRealTimeCheckoutPropensityScorerService.class.getName());
    private final Map<String, CustomerRealTimeCheckoutPropensityScorerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CustomerRealTimeCheckoutPropensityScorerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CustomerRealTimeCheckoutPropensityScorerModel model = new CustomerRealTimeCheckoutPropensityScorerModel(
                mutId,
    ...
    public CustomerRealTimeCheckoutPropensityScorerModel processIncomingMutation(CustomerRealTimeCheckoutPropensityScorerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CustomerRealTimeCheckoutPropensityScorerService");
    ...
    public Optional<CustomerRealTimeCheckoutPropensityScorerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CustomerRealTimeCheckoutPropensityScorerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CustomerRealTimeCheckoutPropensityScorerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CustomerRealTimeCheckoutPropensityScorerModel model = storageCache.get(mutationId);
        if (model != null) {
