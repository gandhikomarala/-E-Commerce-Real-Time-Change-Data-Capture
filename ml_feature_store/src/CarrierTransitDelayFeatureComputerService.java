...
/**
 * Enterprise Service Processor: CarrierTransitDelayFeatureComputerService.
 * Computes weather-adjusted carrier transit delay feature vectors for estimated delivery dates.
 */
public class CarrierTransitDelayFeatureComputerService {

    private static final Logger LOGGER = Logger.getLogger(CarrierTransitDelayFeatureComputerService.class.getName());
    private final Map<String, CarrierTransitDelayFeatureComputerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CarrierTransitDelayFeatureComputerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CarrierTransitDelayFeatureComputerModel model = new CarrierTransitDelayFeatureComputerModel(
                mutId,
    ...
    public CarrierTransitDelayFeatureComputerModel processIncomingMutation(CarrierTransitDelayFeatureComputerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CarrierTransitDelayFeatureComputerService");
    ...
    public Optional<CarrierTransitDelayFeatureComputerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CarrierTransitDelayFeatureComputerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CarrierTransitDelayFeatureComputerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CarrierTransitDelayFeatureComputerModel model = storageCache.get(mutationId);
        if (model != null) {
