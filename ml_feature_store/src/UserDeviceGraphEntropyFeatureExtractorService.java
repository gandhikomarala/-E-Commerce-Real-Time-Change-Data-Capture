...
/**
 * Enterprise Service Processor: UserDeviceGraphEntropyFeatureExtractorService.
 * Extracts graph entropy and IP subnet sharing features for account security.
 */
public class UserDeviceGraphEntropyFeatureExtractorService {

    private static final Logger LOGGER = Logger.getLogger(UserDeviceGraphEntropyFeatureExtractorService.class.getName());
    private final Map<String, UserDeviceGraphEntropyFeatureExtractorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public UserDeviceGraphEntropyFeatureExtractorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            UserDeviceGraphEntropyFeatureExtractorModel model = new UserDeviceGraphEntropyFeatureExtractorModel(
                mutId,
    ...
    public UserDeviceGraphEntropyFeatureExtractorModel processIncomingMutation(UserDeviceGraphEntropyFeatureExtractorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in UserDeviceGraphEntropyFeatureExtractorService");
    ...
    public Optional<UserDeviceGraphEntropyFeatureExtractorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<UserDeviceGraphEntropyFeatureExtractorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<UserDeviceGraphEntropyFeatureExtractorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        UserDeviceGraphEntropyFeatureExtractorModel model = storageCache.get(mutationId);
        if (model != null) {
