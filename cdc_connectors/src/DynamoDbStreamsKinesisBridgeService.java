...
/**
 * Enterprise Service Processor: DynamoDbStreamsKinesisBridgeService.
 * Consumes AWS DynamoDB Streams shards and converts item mutations into standardized CloudEvents.
 */
public class DynamoDbStreamsKinesisBridgeService {

    private static final Logger LOGGER = Logger.getLogger(DynamoDbStreamsKinesisBridgeService.class.getName());
    private final Map<String, DynamoDbStreamsKinesisBridgeModel> storageCache = new ConcurrentHashMap<>();
    ...
    public DynamoDbStreamsKinesisBridgeService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            DynamoDbStreamsKinesisBridgeModel model = new DynamoDbStreamsKinesisBridgeModel(
                mutId,
    ...
    public DynamoDbStreamsKinesisBridgeModel processIncomingMutation(DynamoDbStreamsKinesisBridgeModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in DynamoDbStreamsKinesisBridgeService");
    ...
    public Optional<DynamoDbStreamsKinesisBridgeModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<DynamoDbStreamsKinesisBridgeModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<DynamoDbStreamsKinesisBridgeModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        DynamoDbStreamsKinesisBridgeModel model = storageCache.get(mutationId);
        if (model != null) {
