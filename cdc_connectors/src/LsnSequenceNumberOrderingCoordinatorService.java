package com.ecommerce.lakehouse.cdc;
...
/**
 * Enterprise Service Processor: LsnSequenceNumberOrderingCoordinatorService.
 * Guarantees strict linearizable causal ordering of mutations using WAL Log Sequence Numbers (LSN).
 */
public class LsnSequenceNumberOrderingCoordinatorService {

    private static final Logger LOGGER = Logger.getLogger(LsnSequenceNumberOrderingCoordinatorService.class.getName());
    private final Map<String, LsnSequenceNumberOrderingCoordinatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public LsnSequenceNumberOrderingCoordinatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            LsnSequenceNumberOrderingCoordinatorModel model = new LsnSequenceNumberOrderingCoordinatorModel(
                mutId,
    ...
    public LsnSequenceNumberOrderingCoordinatorModel processIncomingMutation(LsnSequenceNumberOrderingCoordinatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in LsnSequenceNumberOrderingCoordinatorService");
    ...
    public Optional<LsnSequenceNumberOrderingCoordinatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<LsnSequenceNumberOrderingCoordinatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<LsnSequenceNumberOrderingCoordinatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        LsnSequenceNumberOrderingCoordinatorModel model = storageCache.get(mutationId);
        if (model != null) {
