...
/**
 * Enterprise Service Processor: DebeziumAvroSchemaEvolutionRegistryService.
 * Synchronizes dynamic table column additions and schema mutations with Confluent Schema Registry.
 */
public class DebeziumAvroSchemaEvolutionRegistryService {

    private static final Logger LOGGER = Logger.getLogger(DebeziumAvroSchemaEvolutionRegistryService.class.getName());
    private final Map<String, DebeziumAvroSchemaEvolutionRegistryModel> storageCache = new ConcurrentHashMap<>();
    ...
    public DebeziumAvroSchemaEvolutionRegistryService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            DebeziumAvroSchemaEvolutionRegistryModel model = new DebeziumAvroSchemaEvolutionRegistryModel(
                mutId,
    ...
    public DebeziumAvroSchemaEvolutionRegistryModel processIncomingMutation(DebeziumAvroSchemaEvolutionRegistryModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in DebeziumAvroSchemaEvolutionRegistryService");
    ...
    public Optional<DebeziumAvroSchemaEvolutionRegistryModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<DebeziumAvroSchemaEvolutionRegistryModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<DebeziumAvroSchemaEvolutionRegistryModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        DebeziumAvroSchemaEvolutionRegistryModel model = storageCache.get(mutationId);
        if (model != null) {
