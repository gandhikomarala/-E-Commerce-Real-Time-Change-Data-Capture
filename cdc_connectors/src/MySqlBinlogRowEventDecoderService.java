...
/**
 * Enterprise Service Processor: MySqlBinlogRowEventDecoderService.
 * Consumes MySQL binary row-level mutations (TABLE_MAP, WRITE_ROWS, UPDATE_ROWS, DELETE_ROWS).
 */
public class MySqlBinlogRowEventDecoderService {

    private static final Logger LOGGER = Logger.getLogger(MySqlBinlogRowEventDecoderService.class.getName());
    private final Map<String, MySqlBinlogRowEventDecoderModel> storageCache = new ConcurrentHashMap<>();
    ...
    public MySqlBinlogRowEventDecoderService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            MySqlBinlogRowEventDecoderModel model = new MySqlBinlogRowEventDecoderModel(
                mutId,
    ...
    public MySqlBinlogRowEventDecoderModel processIncomingMutation(MySqlBinlogRowEventDecoderModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in MySqlBinlogRowEventDecoderService");
    ...
    public Optional<MySqlBinlogRowEventDecoderModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<MySqlBinlogRowEventDecoderModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<MySqlBinlogRowEventDecoderModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        MySqlBinlogRowEventDecoderModel model = storageCache.get(mutationId);
        if (model != null) {
