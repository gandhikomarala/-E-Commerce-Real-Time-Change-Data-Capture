...
/**
 * Enterprise Service Processor: KafkaConnectDistributedWorkerClusterService.
 * Manages Kafka Connect distributed worker cluster configurations, task rebalancing, and offsets.
 */
public class KafkaConnectDistributedWorkerClusterService {

    private static final Logger LOGGER = Logger.getLogger(KafkaConnectDistributedWorkerClusterService.class.getName());
    private final Map<String, KafkaConnectDistributedWorkerClusterModel> storageCache = new ConcurrentHashMap<>();
    ...
    public KafkaConnectDistributedWorkerClusterService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            KafkaConnectDistributedWorkerClusterModel model = new KafkaConnectDistributedWorkerClusterModel(
                mutId,
    ...
    public KafkaConnectDistributedWorkerClusterModel processIncomingMutation(KafkaConnectDistributedWorkerClusterModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in KafkaConnectDistributedWorkerClusterService");
    ...
    public Optional<KafkaConnectDistributedWorkerClusterModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<KafkaConnectDistributedWorkerClusterModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<KafkaConnectDistributedWorkerClusterModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        KafkaConnectDistributedWorkerClusterModel model = storageCache.get(mutationId);
        if (model != null) {
