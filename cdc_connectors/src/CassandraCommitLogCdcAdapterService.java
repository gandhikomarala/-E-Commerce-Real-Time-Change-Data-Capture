...
/**
 * Enterprise Service Processor: CassandraCommitLogCdcAdapterService.
 * Captures commitlog mutations across distributed Apache Cassandra cluster partition keys.
 */
public class CassandraCommitLogCdcAdapterService {

    private static final Logger LOGGER = Logger.getLogger(CassandraCommitLogCdcAdapterService.class.getName());
    private final Map<String, CassandraCommitLogCdcAdapterModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CassandraCommitLogCdcAdapterService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CassandraCommitLogCdcAdapterModel model = new CassandraCommitLogCdcAdapterModel(
                mutId,
    ...
    public CassandraCommitLogCdcAdapterModel processIncomingMutation(CassandraCommitLogCdcAdapterModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CassandraCommitLogCdcAdapterService");
    ...
    public Optional<CassandraCommitLogCdcAdapterModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CassandraCommitLogCdcAdapterModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CassandraCommitLogCdcAdapterModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CassandraCommitLogCdcAdapterModel model = storageCache.get(mutationId);
        if (model != null) {
