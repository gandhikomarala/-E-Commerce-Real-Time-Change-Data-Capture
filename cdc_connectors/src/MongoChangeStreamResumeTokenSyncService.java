...
/**
 * Enterprise Service Processor: MongoChangeStreamResumeTokenSyncService.
 * Streams MongoDB 4.4+ change streams with resume token persistent checkpointing.
 */
public class MongoChangeStreamResumeTokenSyncService {

    private static final Logger LOGGER = Logger.getLogger(MongoChangeStreamResumeTokenSyncService.class.getName());
    private final Map<String, MongoChangeStreamResumeTokenSyncModel> storageCache = new ConcurrentHashMap<>();
    ...
    public MongoChangeStreamResumeTokenSyncService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            MongoChangeStreamResumeTokenSyncModel model = new MongoChangeStreamResumeTokenSyncModel(
                mutId,
    ...
    public MongoChangeStreamResumeTokenSyncModel processIncomingMutation(MongoChangeStreamResumeTokenSyncModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in MongoChangeStreamResumeTokenSyncService");
    ...
    public Optional<MongoChangeStreamResumeTokenSyncModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<MongoChangeStreamResumeTokenSyncModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<MongoChangeStreamResumeTokenSyncModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        MongoChangeStreamResumeTokenSyncModel model = storageCache.get(mutationId);
        if (model != null) {
