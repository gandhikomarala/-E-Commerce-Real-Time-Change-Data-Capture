...
/**
 * Enterprise Service Processor: PostgresWalReplicationStreamerService.
 * Extracts PostgreSQL Write-Ahead Log (WAL) logical decoding events via pgoutput plugin.
 */
public class PostgresWalReplicationStreamerService {

    private static final Logger LOGGER = Logger.getLogger(PostgresWalReplicationStreamerService.class.getName());
    private final Map<String, PostgresWalReplicationStreamerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public PostgresWalReplicationStreamerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            PostgresWalReplicationStreamerModel model = new PostgresWalReplicationStreamerModel(
                mutId,
    ...
    public PostgresWalReplicationStreamerModel processIncomingMutation(PostgresWalReplicationStreamerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in PostgresWalReplicationStreamerService");
    ...
    public Optional<PostgresWalReplicationStreamerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<PostgresWalReplicationStreamerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<PostgresWalReplicationStreamerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        PostgresWalReplicationStreamerModel model = storageCache.get(mutationId);
        if (model != null) {
