...
/**
 * Enterprise Service Processor: DeadLetterQueueMutationQuarantineService.
 * Quarantines malformed, unparseable, or schema-violating CDC payloads into isolated DLQ topic.
 */
public class DeadLetterQueueMutationQuarantineService {

    private static final Logger LOGGER = Logger.getLogger(DeadLetterQueueMutationQuarantineService.class.getName());
    private final Map<String, DeadLetterQueueMutationQuarantineModel> storageCache = new ConcurrentHashMap<>();
    ...
    public DeadLetterQueueMutationQuarantineService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            DeadLetterQueueMutationQuarantineModel model = new DeadLetterQueueMutationQuarantineModel(
                mutId,
    ...
    public DeadLetterQueueMutationQuarantineModel processIncomingMutation(DeadLetterQueueMutationQuarantineModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in DeadLetterQueueMutationQuarantineService");
    ...
    public Optional<DeadLetterQueueMutationQuarantineModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<DeadLetterQueueMutationQuarantineModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<DeadLetterQueueMutationQuarantineModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        DeadLetterQueueMutationQuarantineModel model = storageCache.get(mutationId);
        if (model != null) {
