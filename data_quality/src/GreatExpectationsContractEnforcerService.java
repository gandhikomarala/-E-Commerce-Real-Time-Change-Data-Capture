...
/**
 * Enterprise Service Processor: GreatExpectationsContractEnforcerService.
 * Runs automated suite of statistical data assertions before promoting Silver records to Gold.
 */
public class GreatExpectationsContractEnforcerService {

    private static final Logger LOGGER = Logger.getLogger(GreatExpectationsContractEnforcerService.class.getName());
    private final Map<String, GreatExpectationsContractEnforcerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public GreatExpectationsContractEnforcerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            GreatExpectationsContractEnforcerModel model = new GreatExpectationsContractEnforcerModel(
                mutId,
    ...
    public GreatExpectationsContractEnforcerModel processIncomingMutation(GreatExpectationsContractEnforcerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in GreatExpectationsContractEnforcerService");
    ...
    public Optional<GreatExpectationsContractEnforcerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<GreatExpectationsContractEnforcerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<GreatExpectationsContractEnforcerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        GreatExpectationsContractEnforcerModel model = storageCache.get(mutationId);
        if (model != null) {
