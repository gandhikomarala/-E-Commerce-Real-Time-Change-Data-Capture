...
/**
 * Enterprise Service Processor: NullValueIntegrityViolationAuditorService.
 * Blocks CDC merges containing NULL values in non-nullable business primary or foreign keys.
 */
public class NullValueIntegrityViolationAuditorService {

    private static final Logger LOGGER = Logger.getLogger(NullValueIntegrityViolationAuditorService.class.getName());
    private final Map<String, NullValueIntegrityViolationAuditorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public NullValueIntegrityViolationAuditorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            NullValueIntegrityViolationAuditorModel model = new NullValueIntegrityViolationAuditorModel(
                mutId,
    ...
    public NullValueIntegrityViolationAuditorModel processIncomingMutation(NullValueIntegrityViolationAuditorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in NullValueIntegrityViolationAuditorService");
    ...
    public Optional<NullValueIntegrityViolationAuditorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<NullValueIntegrityViolationAuditorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<NullValueIntegrityViolationAuditorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        NullValueIntegrityViolationAuditorModel model = storageCache.get(mutationId);
        if (model != null) {
