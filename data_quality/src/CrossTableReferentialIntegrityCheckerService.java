package com.ecommerce.lakehouse.quality;
...
/**
 * Enterprise Service Processor: CrossTableReferentialIntegrityCheckerService.
 * Validates that every order_item CDC record references an existing product SKU in Silver layer.
 */
public class CrossTableReferentialIntegrityCheckerService {

    private static final Logger LOGGER = Logger.getLogger(CrossTableReferentialIntegrityCheckerService.class.getName());
    private final Map<String, CrossTableReferentialIntegrityCheckerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CrossTableReferentialIntegrityCheckerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CrossTableReferentialIntegrityCheckerModel model = new CrossTableReferentialIntegrityCheckerModel(
                mutId,
    ...
    public CrossTableReferentialIntegrityCheckerModel processIncomingMutation(CrossTableReferentialIntegrityCheckerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CrossTableReferentialIntegrityCheckerService");
    ...
    public Optional<CrossTableReferentialIntegrityCheckerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CrossTableReferentialIntegrityCheckerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CrossTableReferentialIntegrityCheckerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CrossTableReferentialIntegrityCheckerModel model = storageCache.get(mutationId);
        if (model != null) {
