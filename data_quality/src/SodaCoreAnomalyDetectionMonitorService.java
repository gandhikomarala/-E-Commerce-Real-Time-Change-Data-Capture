...
/**
 * Enterprise Service Processor: SodaCoreAnomalyDetectionMonitorService.
 * Continuously tracks distribution anomalies in order totals, discount percentages, and row counts.
 */
public class SodaCoreAnomalyDetectionMonitorService {

    private static final Logger LOGGER = Logger.getLogger(SodaCoreAnomalyDetectionMonitorService.class.getName());
    private final Map<String, SodaCoreAnomalyDetectionMonitorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public SodaCoreAnomalyDetectionMonitorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            SodaCoreAnomalyDetectionMonitorModel model = new SodaCoreAnomalyDetectionMonitorModel(
                mutId,
    ...
    public SodaCoreAnomalyDetectionMonitorModel processIncomingMutation(SodaCoreAnomalyDetectionMonitorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in SodaCoreAnomalyDetectionMonitorService");
    ...
    public Optional<SodaCoreAnomalyDetectionMonitorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<SodaCoreAnomalyDetectionMonitorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<SodaCoreAnomalyDetectionMonitorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        SodaCoreAnomalyDetectionMonitorModel model = storageCache.get(mutationId);
        if (model != null) {
