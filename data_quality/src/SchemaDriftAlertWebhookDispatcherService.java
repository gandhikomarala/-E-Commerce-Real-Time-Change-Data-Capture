...
/**
 * Enterprise Service Processor: SchemaDriftAlertWebhookDispatcherService.
 * Dispatches instant Slack/PagerDuty alerts when source OLTP schema adds or drops columns.
 */
public class SchemaDriftAlertWebhookDispatcherService {

    private static final Logger LOGGER = Logger.getLogger(SchemaDriftAlertWebhookDispatcherService.class.getName());
    private final Map<String, SchemaDriftAlertWebhookDispatcherModel> storageCache = new ConcurrentHashMap<>();
    ...
    public SchemaDriftAlertWebhookDispatcherService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            SchemaDriftAlertWebhookDispatcherModel model = new SchemaDriftAlertWebhookDispatcherModel(
                mutId,
    ...
    public SchemaDriftAlertWebhookDispatcherModel processIncomingMutation(SchemaDriftAlertWebhookDispatcherModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in SchemaDriftAlertWebhookDispatcherService");
    ...
    public Optional<SchemaDriftAlertWebhookDispatcherModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<SchemaDriftAlertWebhookDispatcherModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<SchemaDriftAlertWebhookDispatcherModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        SchemaDriftAlertWebhookDispatcherModel model = storageCache.get(mutationId);
        if (model != null) {
