...
/**
 * Enterprise Service Processor: PromoDiscountAbuseDetectionMartService.
 * Identifies users creating multiple accounts to exploit welcome promo codes.
 */
public class PromoDiscountAbuseDetectionMartService {

    private static final Logger LOGGER = Logger.getLogger(PromoDiscountAbuseDetectionMartService.class.getName());
    private final Map<String, PromoDiscountAbuseDetectionMartModel> storageCache = new ConcurrentHashMap<>();
    ...
    public PromoDiscountAbuseDetectionMartService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            PromoDiscountAbuseDetectionMartModel model = new PromoDiscountAbuseDetectionMartModel(
                mutId,
    ...
    public PromoDiscountAbuseDetectionMartModel processIncomingMutation(PromoDiscountAbuseDetectionMartModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in PromoDiscountAbuseDetectionMartService");
    ...
    public Optional<PromoDiscountAbuseDetectionMartModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<PromoDiscountAbuseDetectionMartModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<PromoDiscountAbuseDetectionMartModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        PromoDiscountAbuseDetectionMartModel model = storageCache.get(mutationId);
        if (model != null) {
