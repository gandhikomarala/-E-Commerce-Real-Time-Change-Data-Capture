...
/**
 * Enterprise Service Processor: MerchantPayoutSettlementLedgerService.
 * Calculates net merchant payouts, commission deductions, and rolling reserve withholdings.
 */
public class MerchantPayoutSettlementLedgerService {

    private static final Logger LOGGER = Logger.getLogger(MerchantPayoutSettlementLedgerService.class.getName());
    private final Map<String, MerchantPayoutSettlementLedgerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public MerchantPayoutSettlementLedgerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            MerchantPayoutSettlementLedgerModel model = new MerchantPayoutSettlementLedgerModel(
                mutId,
    ...
    public MerchantPayoutSettlementLedgerModel processIncomingMutation(MerchantPayoutSettlementLedgerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in MerchantPayoutSettlementLedgerService");
    ...
    public Optional<MerchantPayoutSettlementLedgerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<MerchantPayoutSettlementLedgerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<MerchantPayoutSettlementLedgerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        MerchantPayoutSettlementLedgerModel model = storageCache.get(mutationId);
        if (model != null) {
