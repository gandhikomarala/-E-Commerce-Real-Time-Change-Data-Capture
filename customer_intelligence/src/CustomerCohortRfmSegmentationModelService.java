...
/**
 * Enterprise Service Processor: CustomerCohortRfmSegmentationModelService.
 * Computes streaming Recency, Frequency, and Monetary (RFM) quintile scores for every customer.
 */
public class CustomerCohortRfmSegmentationModelService {

    private static final Logger LOGGER = Logger.getLogger(CustomerCohortRfmSegmentationModelService.class.getName());
    private final Map<String, CustomerCohortRfmSegmentationModelModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CustomerCohortRfmSegmentationModelService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CustomerCohortRfmSegmentationModelModel model = new CustomerCohortRfmSegmentationModelModel(
                mutId,
    ...
    public CustomerCohortRfmSegmentationModelModel processIncomingMutation(CustomerCohortRfmSegmentationModelModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CustomerCohortRfmSegmentationModelService");
    ...
    public Optional<CustomerCohortRfmSegmentationModelModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CustomerCohortRfmSegmentationModelModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CustomerCohortRfmSegmentationModelModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CustomerCohortRfmSegmentationModelModel model = storageCache.get(mutationId);
        if (model != null) {
