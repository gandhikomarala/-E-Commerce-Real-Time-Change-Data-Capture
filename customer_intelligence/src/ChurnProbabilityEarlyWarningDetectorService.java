...
/**
 * Enterprise Service Processor: ChurnProbabilityEarlyWarningDetectorService.
 * Flags customers whose session frequency has declined by >40% over consecutive 14-day windows.
 */
public class ChurnProbabilityEarlyWarningDetectorService {

    private static final Logger LOGGER = Logger.getLogger(ChurnProbabilityEarlyWarningDetectorService.class.getName());
    private final Map<String, ChurnProbabilityEarlyWarningDetectorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ChurnProbabilityEarlyWarningDetectorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ChurnProbabilityEarlyWarningDetectorModel model = new ChurnProbabilityEarlyWarningDetectorModel(
                mutId,
    ...
    public ChurnProbabilityEarlyWarningDetectorModel processIncomingMutation(ChurnProbabilityEarlyWarningDetectorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ChurnProbabilityEarlyWarningDetectorService");
    ...
    public Optional<ChurnProbabilityEarlyWarningDetectorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ChurnProbabilityEarlyWarningDetectorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ChurnProbabilityEarlyWarningDetectorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ChurnProbabilityEarlyWarningDetectorModel model = storageCache.get(mutationId);
        if (model != null) {
