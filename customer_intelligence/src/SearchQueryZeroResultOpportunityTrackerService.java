...
/**
 * Enterprise Service Processor: SearchQueryZeroResultOpportunityTrackerService.
 * Identifies trending search queries returning zero product results for catalog expansion.
 */
public class SearchQueryZeroResultOpportunityTrackerService {

    private static final Logger LOGGER = Logger.getLogger(SearchQueryZeroResultOpportunityTrackerService.class.getName());
    private final Map<String, SearchQueryZeroResultOpportunityTrackerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public SearchQueryZeroResultOpportunityTrackerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            SearchQueryZeroResultOpportunityTrackerModel model = new SearchQueryZeroResultOpportunityTrackerModel(
                mutId,
    ...
    public SearchQueryZeroResultOpportunityTrackerModel processIncomingMutation(SearchQueryZeroResultOpportunityTrackerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in SearchQueryZeroResultOpportunityTrackerService");
    ...
    public Optional<SearchQueryZeroResultOpportunityTrackerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<SearchQueryZeroResultOpportunityTrackerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<SearchQueryZeroResultOpportunityTrackerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        SearchQueryZeroResultOpportunityTrackerModel model = storageCache.get(mutationId);
        if (model != null) {
