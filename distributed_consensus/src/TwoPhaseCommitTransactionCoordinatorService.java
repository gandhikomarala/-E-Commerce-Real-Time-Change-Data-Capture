...
/**
 * Enterprise Service Processor: TwoPhaseCommitTransactionCoordinatorService.
 * Coordinates two-phase commit transactions between Kafka and Iceberg.
 */
public class TwoPhaseCommitTransactionCoordinatorService {

    private static final Logger LOGGER = Logger.getLogger(TwoPhaseCommitTransactionCoordinatorService.class.getName());
    private final Map<String, TwoPhaseCommitTransactionCoordinatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public TwoPhaseCommitTransactionCoordinatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            TwoPhaseCommitTransactionCoordinatorModel model = new TwoPhaseCommitTransactionCoordinatorModel(
                mutId,
    ...
    public TwoPhaseCommitTransactionCoordinatorModel processIncomingMutation(TwoPhaseCommitTransactionCoordinatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in TwoPhaseCommitTransactionCoordinatorService");
    ...
    public Optional<TwoPhaseCommitTransactionCoordinatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<TwoPhaseCommitTransactionCoordinatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<TwoPhaseCommitTransactionCoordinatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        TwoPhaseCommitTransactionCoordinatorModel model = storageCache.get(mutationId);
        if (model != null) {
