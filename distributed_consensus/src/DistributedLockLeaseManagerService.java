...
/**
 * Enterprise Service Processor: DistributedLockLeaseManagerService.
 * Acquires dynamic distributed leases for table partition compaction locks.
 */
public class DistributedLockLeaseManagerService {

    private static final Logger LOGGER = Logger.getLogger(DistributedLockLeaseManagerService.class.getName());
    private final Map<String, DistributedLockLeaseManagerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public DistributedLockLeaseManagerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            DistributedLockLeaseManagerModel model = new DistributedLockLeaseManagerModel(
                mutId,
    ...
    public DistributedLockLeaseManagerModel processIncomingMutation(DistributedLockLeaseManagerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in DistributedLockLeaseManagerService");
    ...
    public Optional<DistributedLockLeaseManagerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<DistributedLockLeaseManagerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<DistributedLockLeaseManagerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        DistributedLockLeaseManagerModel model = storageCache.get(mutationId);
        if (model != null) {
