...
/**
 * Enterprise Service Processor: EpochWatermarkSynchronizationBarrierService.
 * Synchronizes high-watermark LSN progress across multi-region clusters.
 */
public class EpochWatermarkSynchronizationBarrierService {

    private static final Logger LOGGER = Logger.getLogger(EpochWatermarkSynchronizationBarrierService.class.getName());
    private final Map<String, EpochWatermarkSynchronizationBarrierModel> storageCache = new ConcurrentHashMap<>();
    ...
    public EpochWatermarkSynchronizationBarrierService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            EpochWatermarkSynchronizationBarrierModel model = new EpochWatermarkSynchronizationBarrierModel(
                mutId,
    ...
    public EpochWatermarkSynchronizationBarrierModel processIncomingMutation(EpochWatermarkSynchronizationBarrierModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in EpochWatermarkSynchronizationBarrierService");
    ...
    public Optional<EpochWatermarkSynchronizationBarrierModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<EpochWatermarkSynchronizationBarrierModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<EpochWatermarkSynchronizationBarrierModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        EpochWatermarkSynchronizationBarrierModel model = storageCache.get(mutationId);
        if (model != null) {
