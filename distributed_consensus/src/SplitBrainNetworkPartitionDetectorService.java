...
/**
 * Enterprise Service Processor: SplitBrainNetworkPartitionDetectorService.
 * Detects cross-datacenter network partitions and enforces fencing tokens.
 */
public class SplitBrainNetworkPartitionDetectorService {

    private static final Logger LOGGER = Logger.getLogger(SplitBrainNetworkPartitionDetectorService.class.getName());
    private final Map<String, SplitBrainNetworkPartitionDetectorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public SplitBrainNetworkPartitionDetectorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            SplitBrainNetworkPartitionDetectorModel model = new SplitBrainNetworkPartitionDetectorModel(
                mutId,
    ...
    public SplitBrainNetworkPartitionDetectorModel processIncomingMutation(SplitBrainNetworkPartitionDetectorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in SplitBrainNetworkPartitionDetectorService");
    ...
    public Optional<SplitBrainNetworkPartitionDetectorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<SplitBrainNetworkPartitionDetectorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<SplitBrainNetworkPartitionDetectorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        SplitBrainNetworkPartitionDetectorModel model = storageCache.get(mutationId);
        if (model != null) {
