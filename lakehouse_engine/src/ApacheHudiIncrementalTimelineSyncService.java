...
/**
 * Enterprise Service Processor: ApacheHudiIncrementalTimelineSyncService.
 * Synchronizes Apache Hudi timeline metadata for incremental change data capture (CDC) replay.
 */
public class ApacheHudiIncrementalTimelineSyncService {

    private static final Logger LOGGER = Logger.getLogger(ApacheHudiIncrementalTimelineSyncService.class.getName());
    private final Map<String, ApacheHudiIncrementalTimelineSyncModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ApacheHudiIncrementalTimelineSyncService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ApacheHudiIncrementalTimelineSyncModel model = new ApacheHudiIncrementalTimelineSyncModel(
                mutId,
    ...
    public ApacheHudiIncrementalTimelineSyncModel processIncomingMutation(ApacheHudiIncrementalTimelineSyncModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ApacheHudiIncrementalTimelineSyncService");
    ...
    public Optional<ApacheHudiIncrementalTimelineSyncModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ApacheHudiIncrementalTimelineSyncModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ApacheHudiIncrementalTimelineSyncModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ApacheHudiIncrementalTimelineSyncModel model = storageCache.get(mutationId);
        if (model != null) {
