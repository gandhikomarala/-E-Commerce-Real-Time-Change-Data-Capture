...
/**
 * Enterprise Service Processor: LakehouseMetadataSnapshotCleanerService.
 * Prunes expired metadata snapshots, manifest lists, and orphaned data files across S3 storage.
 */
public class LakehouseMetadataSnapshotCleanerService {

    private static final Logger LOGGER = Logger.getLogger(LakehouseMetadataSnapshotCleanerService.class.getName());
    private final Map<String, LakehouseMetadataSnapshotCleanerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public LakehouseMetadataSnapshotCleanerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            LakehouseMetadataSnapshotCleanerModel model = new LakehouseMetadataSnapshotCleanerModel(
                mutId,
    ...
    public LakehouseMetadataSnapshotCleanerModel processIncomingMutation(LakehouseMetadataSnapshotCleanerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in LakehouseMetadataSnapshotCleanerService");
    ...
    public Optional<LakehouseMetadataSnapshotCleanerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<LakehouseMetadataSnapshotCleanerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<LakehouseMetadataSnapshotCleanerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        LakehouseMetadataSnapshotCleanerModel model = storageCache.get(mutationId);
        if (model != null) {
