...
/**
 * Enterprise Service Processor: ApacheIcebergPartitionWriterService.
 * Writes ACID transactional Parquet files into Apache Iceberg table format with snapshot isolation.
 */
public class ApacheIcebergPartitionWriterService {

    private static final Logger LOGGER = Logger.getLogger(ApacheIcebergPartitionWriterService.class.getName());
    private final Map<String, ApacheIcebergPartitionWriterModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ApacheIcebergPartitionWriterService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ApacheIcebergPartitionWriterModel model = new ApacheIcebergPartitionWriterModel(
                mutId,
    ...
    public ApacheIcebergPartitionWriterModel processIncomingMutation(ApacheIcebergPartitionWriterModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ApacheIcebergPartitionWriterService");
    ...
    public Optional<ApacheIcebergPartitionWriterModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ApacheIcebergPartitionWriterModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ApacheIcebergPartitionWriterModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ApacheIcebergPartitionWriterModel model = storageCache.get(mutationId);
        if (model != null) {
