...
/**
 * Enterprise Service Processor: EffectiveDateRangeOverlapValidatorService.
 * Guarantees zero temporal overlap between valid_from and valid_to timestamps across dimension rows.
 */
public class EffectiveDateRangeOverlapValidatorService {

    private static final Logger LOGGER = Logger.getLogger(EffectiveDateRangeOverlapValidatorService.class.getName());
    private final Map<String, EffectiveDateRangeOverlapValidatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public EffectiveDateRangeOverlapValidatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            EffectiveDateRangeOverlapValidatorModel model = new EffectiveDateRangeOverlapValidatorModel(
                mutId,
    ...
    public EffectiveDateRangeOverlapValidatorModel processIncomingMutation(EffectiveDateRangeOverlapValidatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in EffectiveDateRangeOverlapValidatorService");
    ...
    public Optional<EffectiveDateRangeOverlapValidatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<EffectiveDateRangeOverlapValidatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<EffectiveDateRangeOverlapValidatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        EffectiveDateRangeOverlapValidatorModel model = storageCache.get(mutationId);
        if (model != null) {
