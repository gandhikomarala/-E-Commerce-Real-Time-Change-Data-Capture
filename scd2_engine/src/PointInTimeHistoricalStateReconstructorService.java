package com.ecommerce.lakehouse.scd2;
...
/**
 * Enterprise Service Processor: PointInTimeHistoricalStateReconstructorService.
 * Reconstructs exact customer and product dimension attributes as of any arbitrary historical timestamp.
 */
public class PointInTimeHistoricalStateReconstructorService {

    private static final Logger LOGGER = Logger.getLogger(PointInTimeHistoricalStateReconstructorService.class.getName());
    private final Map<String, PointInTimeHistoricalStateReconstructorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public PointInTimeHistoricalStateReconstructorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            PointInTimeHistoricalStateReconstructorModel model = new PointInTimeHistoricalStateReconstructorModel(
                mutId,
    ...
    public PointInTimeHistoricalStateReconstructorModel processIncomingMutation(PointInTimeHistoricalStateReconstructorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in PointInTimeHistoricalStateReconstructorService");
    ...
    public Optional<PointInTimeHistoricalStateReconstructorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<PointInTimeHistoricalStateReconstructorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<PointInTimeHistoricalStateReconstructorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        PointInTimeHistoricalStateReconstructorModel model = storageCache.get(mutationId);
        if (model != null) {
