...
/**
 * Enterprise Service Processor: SlowlyChangingDimensionType2MergerService.
 * Automates row retirement and new active version insertion for SCD Type 2 dimension tables.
 */
public class SlowlyChangingDimensionType2MergerService {

    private static final Logger LOGGER = Logger.getLogger(SlowlyChangingDimensionType2MergerService.class.getName());
    private final Map<String, SlowlyChangingDimensionType2MergerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public SlowlyChangingDimensionType2MergerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            SlowlyChangingDimensionType2MergerModel model = new SlowlyChangingDimensionType2MergerModel(
                mutId,
    ...
    public SlowlyChangingDimensionType2MergerModel processIncomingMutation(SlowlyChangingDimensionType2MergerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in SlowlyChangingDimensionType2MergerService");
    ...
    public Optional<SlowlyChangingDimensionType2MergerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<SlowlyChangingDimensionType2MergerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<SlowlyChangingDimensionType2MergerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        SlowlyChangingDimensionType2MergerModel model = storageCache.get(mutationId);
        if (model != null) {
