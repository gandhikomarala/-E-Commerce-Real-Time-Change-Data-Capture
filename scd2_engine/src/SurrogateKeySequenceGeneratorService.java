...
/**
 * Enterprise Service Processor: SurrogateKeySequenceGeneratorService.
 * Generates 64-bit monotonically increasing surrogate keys for dimension table joins.
 */
public class SurrogateKeySequenceGeneratorService {

    private static final Logger LOGGER = Logger.getLogger(SurrogateKeySequenceGeneratorService.class.getName());
    private final Map<String, SurrogateKeySequenceGeneratorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public SurrogateKeySequenceGeneratorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            SurrogateKeySequenceGeneratorModel model = new SurrogateKeySequenceGeneratorModel(
                mutId,
    ...
    public SurrogateKeySequenceGeneratorModel processIncomingMutation(SurrogateKeySequenceGeneratorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in SurrogateKeySequenceGeneratorService");
    ...
    public Optional<SurrogateKeySequenceGeneratorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<SurrogateKeySequenceGeneratorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<SurrogateKeySequenceGeneratorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        SurrogateKeySequenceGeneratorModel model = storageCache.get(mutationId);
        if (model != null) {
