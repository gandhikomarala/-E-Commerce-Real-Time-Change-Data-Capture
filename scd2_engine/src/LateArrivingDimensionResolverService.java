...
/**
 * Enterprise Service Processor: LateArrivingDimensionResolverService.
 * Handles out-of-order fact records arriving before corresponding dimension entities are created.
 */
public class LateArrivingDimensionResolverService {

    private static final Logger LOGGER = Logger.getLogger(LateArrivingDimensionResolverService.class.getName());
    private final Map<String, LateArrivingDimensionResolverModel> storageCache = new ConcurrentHashMap<>();
    ...
    public LateArrivingDimensionResolverService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            LateArrivingDimensionResolverModel model = new LateArrivingDimensionResolverModel(
                mutId,
    ...
    public LateArrivingDimensionResolverModel processIncomingMutation(LateArrivingDimensionResolverModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in LateArrivingDimensionResolverService");
    ...
    public Optional<LateArrivingDimensionResolverModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<LateArrivingDimensionResolverModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<LateArrivingDimensionResolverModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        LateArrivingDimensionResolverModel model = storageCache.get(mutationId);
        if (model != null) {
