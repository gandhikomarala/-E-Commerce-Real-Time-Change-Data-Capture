...
/**
 * Enterprise Service Processor: LoyaltyRewardPointBurnAccrualLedgerService.
 * Maintains double-entry ledger of customer loyalty reward points earned and redeemed.
 */
public class LoyaltyRewardPointBurnAccrualLedgerService {

    private static final Logger LOGGER = Logger.getLogger(LoyaltyRewardPointBurnAccrualLedgerService.class.getName());
    private final Map<String, LoyaltyRewardPointBurnAccrualLedgerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public LoyaltyRewardPointBurnAccrualLedgerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            LoyaltyRewardPointBurnAccrualLedgerModel model = new LoyaltyRewardPointBurnAccrualLedgerModel(
                mutId,
    ...
    public LoyaltyRewardPointBurnAccrualLedgerModel processIncomingMutation(LoyaltyRewardPointBurnAccrualLedgerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in LoyaltyRewardPointBurnAccrualLedgerService");
    ...
    public Optional<LoyaltyRewardPointBurnAccrualLedgerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<LoyaltyRewardPointBurnAccrualLedgerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<LoyaltyRewardPointBurnAccrualLedgerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        LoyaltyRewardPointBurnAccrualLedgerModel model = storageCache.get(mutationId);
        if (model != null) {
