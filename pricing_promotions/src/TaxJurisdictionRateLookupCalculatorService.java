...
/**
 * Enterprise Service Processor: TaxJurisdictionRateLookupCalculatorService.
 * Calculates municipal, state, and national sales tax rates across 12,000+ US postal jurisdictions.
 */
public class TaxJurisdictionRateLookupCalculatorService {

    private static final Logger LOGGER = Logger.getLogger(TaxJurisdictionRateLookupCalculatorService.class.getName());
    private final Map<String, TaxJurisdictionRateLookupCalculatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public TaxJurisdictionRateLookupCalculatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            TaxJurisdictionRateLookupCalculatorModel model = new TaxJurisdictionRateLookupCalculatorModel(
                mutId,
    ...
    public TaxJurisdictionRateLookupCalculatorModel processIncomingMutation(TaxJurisdictionRateLookupCalculatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in TaxJurisdictionRateLookupCalculatorService");
    ...
    public Optional<TaxJurisdictionRateLookupCalculatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<TaxJurisdictionRateLookupCalculatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<TaxJurisdictionRateLookupCalculatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        TaxJurisdictionRateLookupCalculatorModel model = storageCache.get(mutationId);
        if (model != null) {
