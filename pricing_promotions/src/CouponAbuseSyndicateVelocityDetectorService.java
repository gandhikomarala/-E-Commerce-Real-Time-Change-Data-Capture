...
/**
 * Enterprise Service Processor: CouponAbuseSyndicateVelocityDetectorService.
 * Identifies bot syndicates generating disposable email accounts to exhaust single-use coupons.
 */
public class CouponAbuseSyndicateVelocityDetectorService {

    private static final Logger LOGGER = Logger.getLogger(CouponAbuseSyndicateVelocityDetectorService.class.getName());
    private final Map<String, CouponAbuseSyndicateVelocityDetectorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CouponAbuseSyndicateVelocityDetectorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CouponAbuseSyndicateVelocityDetectorModel model = new CouponAbuseSyndicateVelocityDetectorModel(
                mutId,
    ...
    public CouponAbuseSyndicateVelocityDetectorModel processIncomingMutation(CouponAbuseSyndicateVelocityDetectorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CouponAbuseSyndicateVelocityDetectorService");
    ...
    public Optional<CouponAbuseSyndicateVelocityDetectorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CouponAbuseSyndicateVelocityDetectorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CouponAbuseSyndicateVelocityDetectorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CouponAbuseSyndicateVelocityDetectorModel model = storageCache.get(mutationId);
        if (model != null) {
