...
/**
 * Enterprise Service Processor: CompetitorPriceScrapeMatchingEngineService.
 * Normalizes and matches competitor marketplace catalog prices using embedding similarity.
 */
public class CompetitorPriceScrapeMatchingEngineService {

    private static final Logger LOGGER = Logger.getLogger(CompetitorPriceScrapeMatchingEngineService.class.getName());
    private final Map<String, CompetitorPriceScrapeMatchingEngineModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CompetitorPriceScrapeMatchingEngineService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CompetitorPriceScrapeMatchingEngineModel model = new CompetitorPriceScrapeMatchingEngineModel(
                mutId,
    ...
    public CompetitorPriceScrapeMatchingEngineModel processIncomingMutation(CompetitorPriceScrapeMatchingEngineModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CompetitorPriceScrapeMatchingEngineService");
    ...
    public Optional<CompetitorPriceScrapeMatchingEngineModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CompetitorPriceScrapeMatchingEngineModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CompetitorPriceScrapeMatchingEngineModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CompetitorPriceScrapeMatchingEngineModel model = storageCache.get(mutationId);
        if (model != null) {
