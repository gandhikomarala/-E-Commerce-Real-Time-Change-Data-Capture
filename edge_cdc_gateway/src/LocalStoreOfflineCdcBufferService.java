...
/**
 * Enterprise Service Processor: LocalStoreOfflineCdcBufferService.
 * Buffers local point-of-sale terminal mutations during internet uplink outages.
 */
public class LocalStoreOfflineCdcBufferService {

    private static final Logger LOGGER = Logger.getLogger(LocalStoreOfflineCdcBufferService.class.getName());
    private final Map<String, LocalStoreOfflineCdcBufferModel> storageCache = new ConcurrentHashMap<>();
    ...
    public LocalStoreOfflineCdcBufferService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            LocalStoreOfflineCdcBufferModel model = new LocalStoreOfflineCdcBufferModel(
                mutId,
    ...
    public LocalStoreOfflineCdcBufferModel processIncomingMutation(LocalStoreOfflineCdcBufferModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in LocalStoreOfflineCdcBufferService");
    ...
    public Optional<LocalStoreOfflineCdcBufferModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<LocalStoreOfflineCdcBufferModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<LocalStoreOfflineCdcBufferModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        LocalStoreOfflineCdcBufferModel model = storageCache.get(mutationId);
        if (model != null) {
