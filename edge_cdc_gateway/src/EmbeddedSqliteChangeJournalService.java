...
/**
 * Enterprise Service Processor: EmbeddedSqliteChangeJournalService.
 * Maintains SQLite rollback journal for local retail kiosk transactions.
 */
public class EmbeddedSqliteChangeJournalService {

    private static final Logger LOGGER = Logger.getLogger(EmbeddedSqliteChangeJournalService.class.getName());
    private final Map<String, EmbeddedSqliteChangeJournalModel> storageCache = new ConcurrentHashMap<>();
    ...
    public EmbeddedSqliteChangeJournalService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            EmbeddedSqliteChangeJournalModel model = new EmbeddedSqliteChangeJournalModel(
                mutId,
    ...
    public EmbeddedSqliteChangeJournalModel processIncomingMutation(EmbeddedSqliteChangeJournalModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in EmbeddedSqliteChangeJournalService");
    ...
    public Optional<EmbeddedSqliteChangeJournalModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<EmbeddedSqliteChangeJournalModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<EmbeddedSqliteChangeJournalModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        EmbeddedSqliteChangeJournalModel model = storageCache.get(mutationId);
        if (model != null) {
