...
/**
 * Enterprise Service Processor: EdgeCompactionBatchOptimizerService.
 * Performs micro-compaction of Parquet rows on edge gateway hardware.
 */
public class EdgeCompactionBatchOptimizerService {

    private static final Logger LOGGER = Logger.getLogger(EdgeCompactionBatchOptimizerService.class.getName());
    private final Map<String, EdgeCompactionBatchOptimizerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public EdgeCompactionBatchOptimizerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            EdgeCompactionBatchOptimizerModel model = new EdgeCompactionBatchOptimizerModel(
                mutId,
    ...
    public EdgeCompactionBatchOptimizerModel processIncomingMutation(EdgeCompactionBatchOptimizerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in EdgeCompactionBatchOptimizerService");
    ...
    public Optional<EdgeCompactionBatchOptimizerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<EdgeCompactionBatchOptimizerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<EdgeCompactionBatchOptimizerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        EdgeCompactionBatchOptimizerModel model = storageCache.get(mutationId);
        if (model != null) {
