...
/**
 * Enterprise Service Processor: MqttTelemetryIngressEndpointService.
 * Consumes IoT RFID pallet scanner events over lightweight MQTT protocol.
 */
public class MqttTelemetryIngressEndpointService {

    private static final Logger LOGGER = Logger.getLogger(MqttTelemetryIngressEndpointService.class.getName());
    private final Map<String, MqttTelemetryIngressEndpointModel> storageCache = new ConcurrentHashMap<>();
    ...
    public MqttTelemetryIngressEndpointService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            MqttTelemetryIngressEndpointModel model = new MqttTelemetryIngressEndpointModel(
                mutId,
    ...
    public MqttTelemetryIngressEndpointModel processIncomingMutation(MqttTelemetryIngressEndpointModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in MqttTelemetryIngressEndpointService");
    ...
    public Optional<MqttTelemetryIngressEndpointModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<MqttTelemetryIngressEndpointModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<MqttTelemetryIngressEndpointModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        MqttTelemetryIngressEndpointModel model = storageCache.get(mutationId);
        if (model != null) {
