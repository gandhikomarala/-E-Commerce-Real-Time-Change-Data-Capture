package com.ecommerce.lakehouse.serving;
...
/**
 * Enterprise Service Processor: WebSocketRealTimeMutationBroadcasterService.
 * Streams filtered live CDC mutation events to subscribed frontend risk analyst consoles.
 */
public class WebSocketRealTimeMutationBroadcasterService {

    private static final Logger LOGGER = Logger.getLogger(WebSocketRealTimeMutationBroadcasterService.class.getName());
    private final Map<String, WebSocketRealTimeMutationBroadcasterModel> storageCache = new ConcurrentHashMap<>();
    ...
    public WebSocketRealTimeMutationBroadcasterService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            WebSocketRealTimeMutationBroadcasterModel model = new WebSocketRealTimeMutationBroadcasterModel(
                mutId,
    ...
    public WebSocketRealTimeMutationBroadcasterModel processIncomingMutation(WebSocketRealTimeMutationBroadcasterModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in WebSocketRealTimeMutationBroadcasterService");
    ...
    public Optional<WebSocketRealTimeMutationBroadcasterModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<WebSocketRealTimeMutationBroadcasterModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<WebSocketRealTimeMutationBroadcasterModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        WebSocketRealTimeMutationBroadcasterModel model = storageCache.get(mutationId);
        if (model != null) {
