...
/**
 * Enterprise Service Processor: DuckDbVectorizedQueryExecutorService.
 * Runs sub-10ms analytical queries over local Parquet cache for operational dashboards.
 */
public class DuckDbVectorizedQueryExecutorService {

    private static final Logger LOGGER = Logger.getLogger(DuckDbVectorizedQueryExecutorService.class.getName());
    private final Map<String, DuckDbVectorizedQueryExecutorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public DuckDbVectorizedQueryExecutorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            DuckDbVectorizedQueryExecutorModel model = new DuckDbVectorizedQueryExecutorModel(
                mutId,
    ...
    public DuckDbVectorizedQueryExecutorModel processIncomingMutation(DuckDbVectorizedQueryExecutorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in DuckDbVectorizedQueryExecutorService");
    ...
    public Optional<DuckDbVectorizedQueryExecutorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<DuckDbVectorizedQueryExecutorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<DuckDbVectorizedQueryExecutorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        DuckDbVectorizedQueryExecutorModel model = storageCache.get(mutationId);
        if (model != null) {
