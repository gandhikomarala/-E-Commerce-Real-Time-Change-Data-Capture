...
/**
 * Enterprise Service Processor: FastApiCdcMetricsRestServerService.
 * Exposes high-throughput OpenAPI REST endpoints for replication lag, LSN positions, and throughput.
 */
public class FastApiCdcMetricsRestServerService {

    private static final Logger LOGGER = Logger.getLogger(FastApiCdcMetricsRestServerService.class.getName());
    private final Map<String, FastApiCdcMetricsRestServerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public FastApiCdcMetricsRestServerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            FastApiCdcMetricsRestServerModel model = new FastApiCdcMetricsRestServerModel(
                mutId,
    ...
    public FastApiCdcMetricsRestServerModel processIncomingMutation(FastApiCdcMetricsRestServerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in FastApiCdcMetricsRestServerService");
    ...
    public Optional<FastApiCdcMetricsRestServerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<FastApiCdcMetricsRestServerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<FastApiCdcMetricsRestServerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        FastApiCdcMetricsRestServerModel model = storageCache.get(mutationId);
        if (model != null) {
