...
/**
 * Enterprise Service Processor: SilverDeduplicationMergeEngineService.
 * Performs primary-key deduplication and row-version resolution from Bronze CDC change stream.
 */
public class SilverDeduplicationMergeEngineService {

    private static final Logger LOGGER = Logger.getLogger(SilverDeduplicationMergeEngineService.class.getName());
    private final Map<String, SilverDeduplicationMergeEngineModel> storageCache = new ConcurrentHashMap<>();
    ...
    public SilverDeduplicationMergeEngineService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            SilverDeduplicationMergeEngineModel model = new SilverDeduplicationMergeEngineModel(
                mutId,
    ...
    public SilverDeduplicationMergeEngineModel processIncomingMutation(SilverDeduplicationMergeEngineModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in SilverDeduplicationMergeEngineService");
    ...
    public Optional<SilverDeduplicationMergeEngineModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<SilverDeduplicationMergeEngineModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<SilverDeduplicationMergeEngineModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        SilverDeduplicationMergeEngineModel model = storageCache.get(mutationId);
        if (model != null) {
