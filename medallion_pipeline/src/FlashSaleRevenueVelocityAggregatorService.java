...
/**
 * Enterprise Service Processor: FlashSaleRevenueVelocityAggregatorService.
 * Calculates tumbling 5-minute GMV, net checkout velocity, and merchant settlement accruals.
 */
public class FlashSaleRevenueVelocityAggregatorService {

    private static final Logger LOGGER = Logger.getLogger(FlashSaleRevenueVelocityAggregatorService.class.getName());
    private final Map<String, FlashSaleRevenueVelocityAggregatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public FlashSaleRevenueVelocityAggregatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            FlashSaleRevenueVelocityAggregatorModel model = new FlashSaleRevenueVelocityAggregatorModel(
                mutId,
    ...
    public FlashSaleRevenueVelocityAggregatorModel processIncomingMutation(FlashSaleRevenueVelocityAggregatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in FlashSaleRevenueVelocityAggregatorService");
    ...
    public Optional<FlashSaleRevenueVelocityAggregatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<FlashSaleRevenueVelocityAggregatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<FlashSaleRevenueVelocityAggregatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        FlashSaleRevenueVelocityAggregatorModel model = storageCache.get(mutationId);
        if (model != null) {
