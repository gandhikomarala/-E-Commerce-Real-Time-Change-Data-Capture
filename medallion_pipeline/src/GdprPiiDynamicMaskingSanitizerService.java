...
/**
 * Enterprise Service Processor: GdprPiiDynamicMaskingSanitizerService.
 * Applies format-preserving tokenization and pseudonymization on customer PII database columns.
 */
public class GdprPiiDynamicMaskingSanitizerService {

    private static final Logger LOGGER = Logger.getLogger(GdprPiiDynamicMaskingSanitizerService.class.getName());
    private final Map<String, GdprPiiDynamicMaskingSanitizerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public GdprPiiDynamicMaskingSanitizerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            GdprPiiDynamicMaskingSanitizerModel model = new GdprPiiDynamicMaskingSanitizerModel(
                mutId,
    ...
    public GdprPiiDynamicMaskingSanitizerModel processIncomingMutation(GdprPiiDynamicMaskingSanitizerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in GdprPiiDynamicMaskingSanitizerService");
    ...
    public Optional<GdprPiiDynamicMaskingSanitizerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<GdprPiiDynamicMaskingSanitizerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<GdprPiiDynamicMaskingSanitizerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        GdprPiiDynamicMaskingSanitizerModel model = storageCache.get(mutationId);
        if (model != null) {
