...
/**
 * Enterprise Service Processor: OrderFulfillmentSlaBreachMonitorService.
 * Tracks duration between ORDER_PLACED and WAREHOUSE_DISPATCHED against regional SLAs.
 */
public class OrderFulfillmentSlaBreachMonitorService {

    private static final Logger LOGGER = Logger.getLogger(OrderFulfillmentSlaBreachMonitorService.class.getName());
    private final Map<String, OrderFulfillmentSlaBreachMonitorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public OrderFulfillmentSlaBreachMonitorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            OrderFulfillmentSlaBreachMonitorModel model = new OrderFulfillmentSlaBreachMonitorModel(
                mutId,
    ...
    public OrderFulfillmentSlaBreachMonitorModel processIncomingMutation(OrderFulfillmentSlaBreachMonitorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in OrderFulfillmentSlaBreachMonitorService");
    ...
    public Optional<OrderFulfillmentSlaBreachMonitorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<OrderFulfillmentSlaBreachMonitorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<OrderFulfillmentSlaBreachMonitorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        OrderFulfillmentSlaBreachMonitorModel model = storageCache.get(mutationId);
        if (model != null) {
