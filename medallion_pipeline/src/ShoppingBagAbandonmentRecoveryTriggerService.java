...
/**
 * Enterprise Service Processor: ShoppingBagAbandonmentRecoveryTriggerService.
 * Identifies shopping cart mutations without corresponding checkout orders within 30-minute window.
 */
public class ShoppingBagAbandonmentRecoveryTriggerService {

    private static final Logger LOGGER = Logger.getLogger(ShoppingBagAbandonmentRecoveryTriggerService.class.getName());
    private final Map<String, ShoppingBagAbandonmentRecoveryTriggerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ShoppingBagAbandonmentRecoveryTriggerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ShoppingBagAbandonmentRecoveryTriggerModel model = new ShoppingBagAbandonmentRecoveryTriggerModel(
                mutId,
    ...
    public ShoppingBagAbandonmentRecoveryTriggerModel processIncomingMutation(ShoppingBagAbandonmentRecoveryTriggerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ShoppingBagAbandonmentRecoveryTriggerService");
    ...
    public Optional<ShoppingBagAbandonmentRecoveryTriggerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ShoppingBagAbandonmentRecoveryTriggerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ShoppingBagAbandonmentRecoveryTriggerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ShoppingBagAbandonmentRecoveryTriggerModel model = storageCache.get(mutationId);
        if (model != null) {
