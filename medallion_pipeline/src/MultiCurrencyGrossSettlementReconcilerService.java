...
/**
 * Enterprise Service Processor: MultiCurrencyGrossSettlementReconcilerService.
 * Reconciles multi-currency checkout authorizations against acquirer settlement batches.
 */
public class MultiCurrencyGrossSettlementReconcilerService {

    private static final Logger LOGGER = Logger.getLogger(MultiCurrencyGrossSettlementReconcilerService.class.getName());
    private final Map<String, MultiCurrencyGrossSettlementReconcilerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public MultiCurrencyGrossSettlementReconcilerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            MultiCurrencyGrossSettlementReconcilerModel model = new MultiCurrencyGrossSettlementReconcilerModel(
                mutId,
    ...
    public MultiCurrencyGrossSettlementReconcilerModel processIncomingMutation(MultiCurrencyGrossSettlementReconcilerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in MultiCurrencyGrossSettlementReconcilerService");
    ...
    public Optional<MultiCurrencyGrossSettlementReconcilerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<MultiCurrencyGrossSettlementReconcilerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<MultiCurrencyGrossSettlementReconcilerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        MultiCurrencyGrossSettlementReconcilerModel model = storageCache.get(mutationId);
        if (model != null) {
