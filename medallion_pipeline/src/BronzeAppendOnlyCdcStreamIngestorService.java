...
/**
 * Enterprise Service Processor: BronzeAppendOnlyCdcStreamIngestorService.
 * Ingests raw immutable CDC payloads into Bronze partition landing zone with ingestion watermarks.
 */
public class BronzeAppendOnlyCdcStreamIngestorService {

    private static final Logger LOGGER = Logger.getLogger(BronzeAppendOnlyCdcStreamIngestorService.class.getName());
    private final Map<String, BronzeAppendOnlyCdcStreamIngestorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public BronzeAppendOnlyCdcStreamIngestorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            BronzeAppendOnlyCdcStreamIngestorModel model = new BronzeAppendOnlyCdcStreamIngestorModel(
                mutId,
    ...
    public BronzeAppendOnlyCdcStreamIngestorModel processIncomingMutation(BronzeAppendOnlyCdcStreamIngestorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in BronzeAppendOnlyCdcStreamIngestorService");
    ...
    public Optional<BronzeAppendOnlyCdcStreamIngestorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<BronzeAppendOnlyCdcStreamIngestorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<BronzeAppendOnlyCdcStreamIngestorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        BronzeAppendOnlyCdcStreamIngestorModel model = storageCache.get(mutationId);
        if (model != null) {
