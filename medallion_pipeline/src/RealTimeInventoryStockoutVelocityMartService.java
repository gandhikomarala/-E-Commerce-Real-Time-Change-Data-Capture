...
/**
 * Enterprise Service Processor: RealTimeInventoryStockoutVelocityMartService.
 * Aggregates real-time inventory decrement velocity and flags imminent stockouts during flash sales.
 */
public class RealTimeInventoryStockoutVelocityMartService {

    private static final Logger LOGGER = Logger.getLogger(RealTimeInventoryStockoutVelocityMartService.class.getName());
    private final Map<String, RealTimeInventoryStockoutVelocityMartModel> storageCache = new ConcurrentHashMap<>();
    ...
    public RealTimeInventoryStockoutVelocityMartService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            RealTimeInventoryStockoutVelocityMartModel model = new RealTimeInventoryStockoutVelocityMartModel(
                mutId,
    ...
    public RealTimeInventoryStockoutVelocityMartModel processIncomingMutation(RealTimeInventoryStockoutVelocityMartModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in RealTimeInventoryStockoutVelocityMartService");
    ...
    public Optional<RealTimeInventoryStockoutVelocityMartModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<RealTimeInventoryStockoutVelocityMartModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<RealTimeInventoryStockoutVelocityMartModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        RealTimeInventoryStockoutVelocityMartModel model = storageCache.get(mutationId);
        if (model != null) {
