package com.ecommerce.lakehouse.observability;
...
/**
 * Enterprise Service Processor: OpenTelemetryDistributedTraceSpanCollectorService.
 * Traces end-to-end latency span from PostgreSQL commit to Iceberg query readiness.
 */
public class OpenTelemetryDistributedTraceSpanCollectorService {

    private static final Logger LOGGER = Logger.getLogger(OpenTelemetryDistributedTraceSpanCollectorService.class.getName());
    private final Map<String, OpenTelemetryDistributedTraceSpanCollectorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public OpenTelemetryDistributedTraceSpanCollectorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            OpenTelemetryDistributedTraceSpanCollectorModel model = new OpenTelemetryDistributedTraceSpanCollectorModel(
                mutId,
    ...
    public OpenTelemetryDistributedTraceSpanCollectorModel processIncomingMutation(OpenTelemetryDistributedTraceSpanCollectorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in OpenTelemetryDistributedTraceSpanCollectorService");
    ...
    public Optional<OpenTelemetryDistributedTraceSpanCollectorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<OpenTelemetryDistributedTraceSpanCollectorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<OpenTelemetryDistributedTraceSpanCollectorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        OpenTelemetryDistributedTraceSpanCollectorModel model = storageCache.get(mutationId);
        if (model != null) {
