...
/**
 * Enterprise Service Processor: GrafanaDashboardMetadataConfiguratorService.
 * Automates Grafana dashboard JSON model generation and alert rule provisioning.
 */
public class GrafanaDashboardMetadataConfiguratorService {

    private static final Logger LOGGER = Logger.getLogger(GrafanaDashboardMetadataConfiguratorService.class.getName());
    private final Map<String, GrafanaDashboardMetadataConfiguratorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public GrafanaDashboardMetadataConfiguratorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            GrafanaDashboardMetadataConfiguratorModel model = new GrafanaDashboardMetadataConfiguratorModel(
                mutId,
    ...
    public GrafanaDashboardMetadataConfiguratorModel processIncomingMutation(GrafanaDashboardMetadataConfiguratorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in GrafanaDashboardMetadataConfiguratorService");
    ...
    public Optional<GrafanaDashboardMetadataConfiguratorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<GrafanaDashboardMetadataConfiguratorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<GrafanaDashboardMetadataConfiguratorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        GrafanaDashboardMetadataConfiguratorModel model = storageCache.get(mutationId);
        if (model != null) {
