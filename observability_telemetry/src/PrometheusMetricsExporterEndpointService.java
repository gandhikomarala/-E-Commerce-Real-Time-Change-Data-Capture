...
/**
 * Enterprise Service Processor: PrometheusMetricsExporterEndpointService.
 * Exposes OpenMetrics prometheus metrics for mutation rate, LSN lag, and compaction duration.
 */
public class PrometheusMetricsExporterEndpointService {

    private static final Logger LOGGER = Logger.getLogger(PrometheusMetricsExporterEndpointService.class.getName());
    private final Map<String, PrometheusMetricsExporterEndpointModel> storageCache = new ConcurrentHashMap<>();
    ...
    public PrometheusMetricsExporterEndpointService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            PrometheusMetricsExporterEndpointModel model = new PrometheusMetricsExporterEndpointModel(
                mutId,
    ...
    public PrometheusMetricsExporterEndpointModel processIncomingMutation(PrometheusMetricsExporterEndpointModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in PrometheusMetricsExporterEndpointService");
    ...
    public Optional<PrometheusMetricsExporterEndpointModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<PrometheusMetricsExporterEndpointModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<PrometheusMetricsExporterEndpointModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        PrometheusMetricsExporterEndpointModel model = storageCache.get(mutationId);
        if (model != null) {
