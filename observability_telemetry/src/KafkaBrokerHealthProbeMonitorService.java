...
/**
 * Enterprise Service Processor: KafkaBrokerHealthProbeMonitorService.
 * Monitors Kafka broker under-replicated partitions and offline log directories.
 */
public class KafkaBrokerHealthProbeMonitorService {

    private static final Logger LOGGER = Logger.getLogger(KafkaBrokerHealthProbeMonitorService.class.getName());
    private final Map<String, KafkaBrokerHealthProbeMonitorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public KafkaBrokerHealthProbeMonitorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            KafkaBrokerHealthProbeMonitorModel model = new KafkaBrokerHealthProbeMonitorModel(
                mutId,
    ...
    public KafkaBrokerHealthProbeMonitorModel processIncomingMutation(KafkaBrokerHealthProbeMonitorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in KafkaBrokerHealthProbeMonitorService");
    ...
    public Optional<KafkaBrokerHealthProbeMonitorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<KafkaBrokerHealthProbeMonitorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<KafkaBrokerHealthProbeMonitorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        KafkaBrokerHealthProbeMonitorModel model = storageCache.get(mutationId);
        if (model != null) {
