...
/**
 * Enterprise Service Processor: IcebergManifestFileHealthAuditorService.
 * Scans for dangling manifest files, corrupted metadata snapshots, and unreferenced data files.
 */
public class IcebergManifestFileHealthAuditorService {

    private static final Logger LOGGER = Logger.getLogger(IcebergManifestFileHealthAuditorService.class.getName());
    private final Map<String, IcebergManifestFileHealthAuditorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public IcebergManifestFileHealthAuditorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            IcebergManifestFileHealthAuditorModel model = new IcebergManifestFileHealthAuditorModel(
                mutId,
    ...
    public IcebergManifestFileHealthAuditorModel processIncomingMutation(IcebergManifestFileHealthAuditorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in IcebergManifestFileHealthAuditorService");
    ...
    public Optional<IcebergManifestFileHealthAuditorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<IcebergManifestFileHealthAuditorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<IcebergManifestFileHealthAuditorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        IcebergManifestFileHealthAuditorModel model = storageCache.get(mutationId);
        if (model != null) {
