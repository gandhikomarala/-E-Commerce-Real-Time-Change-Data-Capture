...
/**
 * Enterprise Service Processor: ReactiveBackpressureThrottlerService.
 * Throttles ingestion rate when downstream Iceberg S3 commit rate falls behind.
 */
public class ReactiveBackpressureThrottlerService {

    private static final Logger LOGGER = Logger.getLogger(ReactiveBackpressureThrottlerService.class.getName());
    private final Map<String, ReactiveBackpressureThrottlerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ReactiveBackpressureThrottlerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ReactiveBackpressureThrottlerModel model = new ReactiveBackpressureThrottlerModel(
                mutId,
    ...
    public ReactiveBackpressureThrottlerModel processIncomingMutation(ReactiveBackpressureThrottlerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ReactiveBackpressureThrottlerService");
    ...
    public Optional<ReactiveBackpressureThrottlerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ReactiveBackpressureThrottlerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ReactiveBackpressureThrottlerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ReactiveBackpressureThrottlerModel model = storageCache.get(mutationId);
        if (model != null) {
