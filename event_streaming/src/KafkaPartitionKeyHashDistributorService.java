package com.ecommerce.lakehouse.streaming;
...
/**
 * Enterprise Service Processor: KafkaPartitionKeyHashDistributorService.
 * Distributes order mutations by customer_id hash to preserve strict per-entity ordering.
 */
public class KafkaPartitionKeyHashDistributorService {

    private static final Logger LOGGER = Logger.getLogger(KafkaPartitionKeyHashDistributorService.class.getName());
    private final Map<String, KafkaPartitionKeyHashDistributorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public KafkaPartitionKeyHashDistributorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            KafkaPartitionKeyHashDistributorModel model = new KafkaPartitionKeyHashDistributorModel(
                mutId,
    ...
    public KafkaPartitionKeyHashDistributorModel processIncomingMutation(KafkaPartitionKeyHashDistributorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in KafkaPartitionKeyHashDistributorService");
    ...
    public Optional<KafkaPartitionKeyHashDistributorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<KafkaPartitionKeyHashDistributorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<KafkaPartitionKeyHashDistributorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        KafkaPartitionKeyHashDistributorModel model = storageCache.get(mutationId);
        if (model != null) {
