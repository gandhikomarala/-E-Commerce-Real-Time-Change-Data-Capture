...
/**
 * Enterprise Service Processor: KafkaConsumerLagAutoScalerService.
 * Monitors consumer lag metrics and scales Flink taskmanager slots automatically.
 */
public class KafkaConsumerLagAutoScalerService {

    private static final Logger LOGGER = Logger.getLogger(KafkaConsumerLagAutoScalerService.class.getName());
    private final Map<String, KafkaConsumerLagAutoScalerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public KafkaConsumerLagAutoScalerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            KafkaConsumerLagAutoScalerModel model = new KafkaConsumerLagAutoScalerModel(
                mutId,
    ...
    public KafkaConsumerLagAutoScalerModel processIncomingMutation(KafkaConsumerLagAutoScalerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in KafkaConsumerLagAutoScalerService");
    ...
    public Optional<KafkaConsumerLagAutoScalerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<KafkaConsumerLagAutoScalerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<KafkaConsumerLagAutoScalerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        KafkaConsumerLagAutoScalerModel model = storageCache.get(mutationId);
        if (model != null) {
