...
/**
 * Enterprise Service Processor: ExactlyOnceTransactionalProducerService.
 * Coordinates two-phase commit Kafka producer transactions with Flink state checkpoints.
 */
public class ExactlyOnceTransactionalProducerService {

    private static final Logger LOGGER = Logger.getLogger(ExactlyOnceTransactionalProducerService.class.getName());
    private final Map<String, ExactlyOnceTransactionalProducerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ExactlyOnceTransactionalProducerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ExactlyOnceTransactionalProducerModel model = new ExactlyOnceTransactionalProducerModel(
                mutId,
    ...
    public ExactlyOnceTransactionalProducerModel processIncomingMutation(ExactlyOnceTransactionalProducerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ExactlyOnceTransactionalProducerService");
    ...
    public Optional<ExactlyOnceTransactionalProducerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ExactlyOnceTransactionalProducerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ExactlyOnceTransactionalProducerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ExactlyOnceTransactionalProducerModel model = storageCache.get(mutationId);
        if (model != null) {
