...
/**
 * Enterprise Service Processor: FlinkStatefulMutationWatermarkerService.
 * Handles late-arriving CDC events with bounded out-of-orderness watermarks.
 */
public class FlinkStatefulMutationWatermarkerService {

    private static final Logger LOGGER = Logger.getLogger(FlinkStatefulMutationWatermarkerService.class.getName());
    private final Map<String, FlinkStatefulMutationWatermarkerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public FlinkStatefulMutationWatermarkerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            FlinkStatefulMutationWatermarkerModel model = new FlinkStatefulMutationWatermarkerModel(
                mutId,
    ...
    public FlinkStatefulMutationWatermarkerModel processIncomingMutation(FlinkStatefulMutationWatermarkerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in FlinkStatefulMutationWatermarkerService");
    ...
    public Optional<FlinkStatefulMutationWatermarkerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<FlinkStatefulMutationWatermarkerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<FlinkStatefulMutationWatermarkerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        FlinkStatefulMutationWatermarkerModel model = storageCache.get(mutationId);
        if (model != null) {
