...
/**
 * Enterprise Service Processor: RefundApprovedLedgerEntryTriggerService.
 * Emits financial double-entry transaction when customer refund request is authorized.
 */
public class RefundApprovedLedgerEntryTriggerService {

    private static final Logger LOGGER = Logger.getLogger(RefundApprovedLedgerEntryTriggerService.class.getName());
    private final Map<String, RefundApprovedLedgerEntryTriggerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public RefundApprovedLedgerEntryTriggerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            RefundApprovedLedgerEntryTriggerModel model = new RefundApprovedLedgerEntryTriggerModel(
                mutId,
    ...
    public RefundApprovedLedgerEntryTriggerModel processIncomingMutation(RefundApprovedLedgerEntryTriggerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in RefundApprovedLedgerEntryTriggerService");
    ...
    public Optional<RefundApprovedLedgerEntryTriggerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<RefundApprovedLedgerEntryTriggerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<RefundApprovedLedgerEntryTriggerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        RefundApprovedLedgerEntryTriggerModel model = storageCache.get(mutationId);
        if (model != null) {
