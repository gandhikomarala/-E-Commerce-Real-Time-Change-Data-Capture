package com.ecommerce.lakehouse.triggers;
...
/**
 * Enterprise Service Processor: FraudScoreHighVelocityInterceptionTriggerService.
 * Triggers immediate payment hold when customer places >5 orders in under 3 minutes.
 */
public class FraudScoreHighVelocityInterceptionTriggerService {

    private static final Logger LOGGER = Logger.getLogger(FraudScoreHighVelocityInterceptionTriggerService.class.getName());
    private final Map<String, FraudScoreHighVelocityInterceptionTriggerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public FraudScoreHighVelocityInterceptionTriggerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            FraudScoreHighVelocityInterceptionTriggerModel model = new FraudScoreHighVelocityInterceptionTriggerModel(
                mutId,
    ...
    public FraudScoreHighVelocityInterceptionTriggerModel processIncomingMutation(FraudScoreHighVelocityInterceptionTriggerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in FraudScoreHighVelocityInterceptionTriggerService");
    ...
    public Optional<FraudScoreHighVelocityInterceptionTriggerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<FraudScoreHighVelocityInterceptionTriggerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<FraudScoreHighVelocityInterceptionTriggerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        FraudScoreHighVelocityInterceptionTriggerModel model = storageCache.get(mutationId);
        if (model != null) {
