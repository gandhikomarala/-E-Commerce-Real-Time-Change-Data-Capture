...
/**
 * Enterprise Service Processor: LowStockAutomatedPurchaseOrderTriggerService.
 * Emits event-driven webhook to supplier ERP when SKU inventory dips below reorder threshold.
 */
public class LowStockAutomatedPurchaseOrderTriggerService {

    private static final Logger LOGGER = Logger.getLogger(LowStockAutomatedPurchaseOrderTriggerService.class.getName());
    private final Map<String, LowStockAutomatedPurchaseOrderTriggerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public LowStockAutomatedPurchaseOrderTriggerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            LowStockAutomatedPurchaseOrderTriggerModel model = new LowStockAutomatedPurchaseOrderTriggerModel(
                mutId,
    ...
    public LowStockAutomatedPurchaseOrderTriggerModel processIncomingMutation(LowStockAutomatedPurchaseOrderTriggerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in LowStockAutomatedPurchaseOrderTriggerService");
    ...
    public Optional<LowStockAutomatedPurchaseOrderTriggerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<LowStockAutomatedPurchaseOrderTriggerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<LowStockAutomatedPurchaseOrderTriggerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        LowStockAutomatedPurchaseOrderTriggerModel model = storageCache.get(mutationId);
        if (model != null) {
