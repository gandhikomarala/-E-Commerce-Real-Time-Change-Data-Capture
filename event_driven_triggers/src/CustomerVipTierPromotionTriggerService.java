...
/**
 * Enterprise Service Processor: CustomerVipTierPromotionTriggerService.
 * Automatically upgrades customer loyalty tier when lifetime GMV crosses $10,000 threshold.
 */
public class CustomerVipTierPromotionTriggerService {

    private static final Logger LOGGER = Logger.getLogger(CustomerVipTierPromotionTriggerService.class.getName());
    private final Map<String, CustomerVipTierPromotionTriggerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CustomerVipTierPromotionTriggerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CustomerVipTierPromotionTriggerModel model = new CustomerVipTierPromotionTriggerModel(
                mutId,
    ...
    public CustomerVipTierPromotionTriggerModel processIncomingMutation(CustomerVipTierPromotionTriggerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CustomerVipTierPromotionTriggerService");
    ...
    public Optional<CustomerVipTierPromotionTriggerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CustomerVipTierPromotionTriggerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CustomerVipTierPromotionTriggerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CustomerVipTierPromotionTriggerModel model = storageCache.get(mutationId);
        if (model != null) {
