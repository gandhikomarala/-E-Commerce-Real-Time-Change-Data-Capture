...
/**
 * Enterprise Service Processor: OrderShippedCustomerNotificationTriggerService.
 * Dispatches instant WhatsApp/SMS notification when order status mutates to SHIPPED.
 */
public class OrderShippedCustomerNotificationTriggerService {

    private static final Logger LOGGER = Logger.getLogger(OrderShippedCustomerNotificationTriggerService.class.getName());
    private final Map<String, OrderShippedCustomerNotificationTriggerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public OrderShippedCustomerNotificationTriggerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            OrderShippedCustomerNotificationTriggerModel model = new OrderShippedCustomerNotificationTriggerModel(
                mutId,
    ...
    public OrderShippedCustomerNotificationTriggerModel processIncomingMutation(OrderShippedCustomerNotificationTriggerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in OrderShippedCustomerNotificationTriggerService");
    ...
    public Optional<OrderShippedCustomerNotificationTriggerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<OrderShippedCustomerNotificationTriggerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<OrderShippedCustomerNotificationTriggerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        OrderShippedCustomerNotificationTriggerModel model = storageCache.get(mutationId);
        if (model != null) {
