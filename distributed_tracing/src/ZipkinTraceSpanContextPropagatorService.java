...
/**
 * Enterprise Service Processor: ZipkinTraceSpanContextPropagatorService.
 * Propagates B3 distributed trace headers across asynchronous CDC worker queues.
 */
public class ZipkinTraceSpanContextPropagatorService {

    private static final Logger LOGGER = Logger.getLogger(ZipkinTraceSpanContextPropagatorService.class.getName());
    private final Map<String, ZipkinTraceSpanContextPropagatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ZipkinTraceSpanContextPropagatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ZipkinTraceSpanContextPropagatorModel model = new ZipkinTraceSpanContextPropagatorModel(
                mutId,
    ...
    public ZipkinTraceSpanContextPropagatorModel processIncomingMutation(ZipkinTraceSpanContextPropagatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ZipkinTraceSpanContextPropagatorService");
    ...
    public Optional<ZipkinTraceSpanContextPropagatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ZipkinTraceSpanContextPropagatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ZipkinTraceSpanContextPropagatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ZipkinTraceSpanContextPropagatorModel model = storageCache.get(mutationId);
        if (model != null) {
