...
/**
 * Enterprise Service Processor: OpenTelemetryContextInjectorFilterService.
 * Injects traceparent W3C context headers into outgoing Kafka record metadata.
 */
public class OpenTelemetryContextInjectorFilterService {

    private static final Logger LOGGER = Logger.getLogger(OpenTelemetryContextInjectorFilterService.class.getName());
    private final Map<String, OpenTelemetryContextInjectorFilterModel> storageCache = new ConcurrentHashMap<>();
    ...
    public OpenTelemetryContextInjectorFilterService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            OpenTelemetryContextInjectorFilterModel model = new OpenTelemetryContextInjectorFilterModel(
                mutId,
    ...
    public OpenTelemetryContextInjectorFilterModel processIncomingMutation(OpenTelemetryContextInjectorFilterModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in OpenTelemetryContextInjectorFilterService");
    ...
    public Optional<OpenTelemetryContextInjectorFilterModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<OpenTelemetryContextInjectorFilterModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<OpenTelemetryContextInjectorFilterModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        OpenTelemetryContextInjectorFilterModel model = storageCache.get(mutationId);
        if (model != null) {
