...
/**
 * Enterprise Service Processor: EndToEndLatencySlaBreachDetectorService.
 * Calculates cumulative end-to-end latency and flags SLA violations > 500ms.
 */
public class EndToEndLatencySlaBreachDetectorService {

    private static final Logger LOGGER = Logger.getLogger(EndToEndLatencySlaBreachDetectorService.class.getName());
    private final Map<String, EndToEndLatencySlaBreachDetectorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public EndToEndLatencySlaBreachDetectorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            EndToEndLatencySlaBreachDetectorModel model = new EndToEndLatencySlaBreachDetectorModel(
                mutId,
    ...
    public EndToEndLatencySlaBreachDetectorModel processIncomingMutation(EndToEndLatencySlaBreachDetectorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in EndToEndLatencySlaBreachDetectorService");
    ...
    public Optional<EndToEndLatencySlaBreachDetectorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<EndToEndLatencySlaBreachDetectorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<EndToEndLatencySlaBreachDetectorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        EndToEndLatencySlaBreachDetectorModel model = storageCache.get(mutationId);
        if (model != null) {
