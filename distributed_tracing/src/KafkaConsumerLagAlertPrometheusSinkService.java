package com.ecommerce.lakehouse.tracing;
...
/**
 * Enterprise Service Processor: KafkaConsumerLagAlertPrometheusSinkService.
 * Exports Kafka consumer group lag metrics into Prometheus TSDB format.
 */
public class KafkaConsumerLagAlertPrometheusSinkService {

    private static final Logger LOGGER = Logger.getLogger(KafkaConsumerLagAlertPrometheusSinkService.class.getName());
    private final Map<String, KafkaConsumerLagAlertPrometheusSinkModel> storageCache = new ConcurrentHashMap<>();
    ...
    public KafkaConsumerLagAlertPrometheusSinkService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            KafkaConsumerLagAlertPrometheusSinkModel model = new KafkaConsumerLagAlertPrometheusSinkModel(
                mutId,
    ...
    public KafkaConsumerLagAlertPrometheusSinkModel processIncomingMutation(KafkaConsumerLagAlertPrometheusSinkModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in KafkaConsumerLagAlertPrometheusSinkService");
    ...
    public Optional<KafkaConsumerLagAlertPrometheusSinkModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<KafkaConsumerLagAlertPrometheusSinkModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<KafkaConsumerLagAlertPrometheusSinkModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        KafkaConsumerLagAlertPrometheusSinkModel model = storageCache.get(mutationId);
        if (model != null) {
