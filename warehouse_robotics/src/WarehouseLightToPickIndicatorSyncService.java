...
/**
 * Enterprise Service Processor: WarehouseLightToPickIndicatorSyncService.
 * Synchronizes Pick-to-Light LED bin display states with warehouse picker terminals.
 */
public class WarehouseLightToPickIndicatorSyncService {

    private static final Logger LOGGER = Logger.getLogger(WarehouseLightToPickIndicatorSyncService.class.getName());
    private final Map<String, WarehouseLightToPickIndicatorSyncModel> storageCache = new ConcurrentHashMap<>();
    ...
    public WarehouseLightToPickIndicatorSyncService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            WarehouseLightToPickIndicatorSyncModel model = new WarehouseLightToPickIndicatorSyncModel(
                mutId,
    ...
    public WarehouseLightToPickIndicatorSyncModel processIncomingMutation(WarehouseLightToPickIndicatorSyncModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in WarehouseLightToPickIndicatorSyncService");
    ...
    public Optional<WarehouseLightToPickIndicatorSyncModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<WarehouseLightToPickIndicatorSyncModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<WarehouseLightToPickIndicatorSyncModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        WarehouseLightToPickIndicatorSyncModel model = storageCache.get(mutationId);
        if (model != null) {
