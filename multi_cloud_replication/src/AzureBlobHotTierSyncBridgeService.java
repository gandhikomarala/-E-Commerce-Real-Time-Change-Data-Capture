...
/**
 * Enterprise Service Processor: AzureBlobHotTierSyncBridgeService.
 * Synchronizes real-time CDC delta changes with Azure Blob Storage Hot Archive tier.
 */
public class AzureBlobHotTierSyncBridgeService {

    private static final Logger LOGGER = Logger.getLogger(AzureBlobHotTierSyncBridgeService.class.getName());
    private final Map<String, AzureBlobHotTierSyncBridgeModel> storageCache = new ConcurrentHashMap<>();
    ...
    public AzureBlobHotTierSyncBridgeService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            AzureBlobHotTierSyncBridgeModel model = new AzureBlobHotTierSyncBridgeModel(
                mutId,
    ...
    public AzureBlobHotTierSyncBridgeModel processIncomingMutation(AzureBlobHotTierSyncBridgeModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in AzureBlobHotTierSyncBridgeService");
    ...
    public Optional<AzureBlobHotTierSyncBridgeModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<AzureBlobHotTierSyncBridgeModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<AzureBlobHotTierSyncBridgeModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        AzureBlobHotTierSyncBridgeModel model = storageCache.get(mutationId);
        if (model != null) {
