...
/**
 * Enterprise Service Processor: MultiRegionActiveActiveDataMeshService.
 * Orchestrates multi-region active-active mesh routing for distributed CDC consumers.
 */
public class MultiRegionActiveActiveDataMeshService {

    private static final Logger LOGGER = Logger.getLogger(MultiRegionActiveActiveDataMeshService.class.getName());
    private final Map<String, MultiRegionActiveActiveDataMeshModel> storageCache = new ConcurrentHashMap<>();
    ...
    public MultiRegionActiveActiveDataMeshService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            MultiRegionActiveActiveDataMeshModel model = new MultiRegionActiveActiveDataMeshModel(
                mutId,
    ...
    public MultiRegionActiveActiveDataMeshModel processIncomingMutation(MultiRegionActiveActiveDataMeshModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in MultiRegionActiveActiveDataMeshService");
    ...
    public Optional<MultiRegionActiveActiveDataMeshModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<MultiRegionActiveActiveDataMeshModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<MultiRegionActiveActiveDataMeshModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        MultiRegionActiveActiveDataMeshModel model = storageCache.get(mutationId);
        if (model != null) {
