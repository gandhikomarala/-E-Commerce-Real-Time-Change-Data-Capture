...
/**
 * Enterprise Service Processor: CloudBandwidthCostOptimizerService.
 * Applies Zstandard compression and batching to minimize inter-region egress transfer fees.
 */
public class CloudBandwidthCostOptimizerService {

    private static final Logger LOGGER = Logger.getLogger(CloudBandwidthCostOptimizerService.class.getName());
    private final Map<String, CloudBandwidthCostOptimizerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CloudBandwidthCostOptimizerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CloudBandwidthCostOptimizerModel model = new CloudBandwidthCostOptimizerModel(
                mutId,
    ...
    public CloudBandwidthCostOptimizerModel processIncomingMutation(CloudBandwidthCostOptimizerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CloudBandwidthCostOptimizerService");
    ...
    public Optional<CloudBandwidthCostOptimizerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CloudBandwidthCostOptimizerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CloudBandwidthCostOptimizerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CloudBandwidthCostOptimizerModel model = storageCache.get(mutationId);
        if (model != null) {
