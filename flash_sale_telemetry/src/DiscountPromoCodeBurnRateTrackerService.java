...
/**
 * Enterprise Service Processor: DiscountPromoCodeBurnRateTrackerService.
 * Tracks remaining marketing promo code budget in real time to prevent over-allocation.
 */
public class DiscountPromoCodeBurnRateTrackerService {

    private static final Logger LOGGER = Logger.getLogger(DiscountPromoCodeBurnRateTrackerService.class.getName());
    private final Map<String, DiscountPromoCodeBurnRateTrackerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public DiscountPromoCodeBurnRateTrackerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            DiscountPromoCodeBurnRateTrackerModel model = new DiscountPromoCodeBurnRateTrackerModel(
                mutId,
    ...
    public DiscountPromoCodeBurnRateTrackerModel processIncomingMutation(DiscountPromoCodeBurnRateTrackerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in DiscountPromoCodeBurnRateTrackerService");
    ...
    public Optional<DiscountPromoCodeBurnRateTrackerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<DiscountPromoCodeBurnRateTrackerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<DiscountPromoCodeBurnRateTrackerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        DiscountPromoCodeBurnRateTrackerModel model = storageCache.get(mutationId);
        if (model != null) {
