...
/**
 * Enterprise Service Processor: PaymentGatewayFailoverSwitchService.
 * Dynamically routes checkout authorizations across Stripe, Adyen, and PayPal during gateway outages.
 */
public class PaymentGatewayFailoverSwitchService {

    private static final Logger LOGGER = Logger.getLogger(PaymentGatewayFailoverSwitchService.class.getName());
    private final Map<String, PaymentGatewayFailoverSwitchModel> storageCache = new ConcurrentHashMap<>();
    ...
    public PaymentGatewayFailoverSwitchService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            PaymentGatewayFailoverSwitchModel model = new PaymentGatewayFailoverSwitchModel(
                mutId,
    ...
    public PaymentGatewayFailoverSwitchModel processIncomingMutation(PaymentGatewayFailoverSwitchModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in PaymentGatewayFailoverSwitchService");
    ...
    public Optional<PaymentGatewayFailoverSwitchModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<PaymentGatewayFailoverSwitchModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<PaymentGatewayFailoverSwitchModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        PaymentGatewayFailoverSwitchModel model = storageCache.get(mutationId);
        if (model != null) {
