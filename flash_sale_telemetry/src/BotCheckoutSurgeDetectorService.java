...
/**
 * Enterprise Service Processor: BotCheckoutSurgeDetectorService.
 * Identifies automated botnet checkout velocity surges and dispatches CAPTCHA challenges.
 */
public class BotCheckoutSurgeDetectorService {

    private static final Logger LOGGER = Logger.getLogger(BotCheckoutSurgeDetectorService.class.getName());
    private final Map<String, BotCheckoutSurgeDetectorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public BotCheckoutSurgeDetectorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            BotCheckoutSurgeDetectorModel model = new BotCheckoutSurgeDetectorModel(
                mutId,
    ...
    public BotCheckoutSurgeDetectorModel processIncomingMutation(BotCheckoutSurgeDetectorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in BotCheckoutSurgeDetectorService");
    ...
    public Optional<BotCheckoutSurgeDetectorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<BotCheckoutSurgeDetectorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<BotCheckoutSurgeDetectorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        BotCheckoutSurgeDetectorModel model = storageCache.get(mutationId);
        if (model != null) {
