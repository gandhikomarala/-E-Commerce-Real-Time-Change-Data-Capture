...
/**
 * Enterprise Service Processor: CartReservationExpiryReleaserService.
 * Releases expired 15-minute shopping cart reservation locks back into available stock stream.
 */
public class CartReservationExpiryReleaserService {

    private static final Logger LOGGER = Logger.getLogger(CartReservationExpiryReleaserService.class.getName());
    private final Map<String, CartReservationExpiryReleaserModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CartReservationExpiryReleaserService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CartReservationExpiryReleaserModel model = new CartReservationExpiryReleaserModel(
                mutId,
    ...
    public CartReservationExpiryReleaserModel processIncomingMutation(CartReservationExpiryReleaserModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CartReservationExpiryReleaserService");
    ...
    public Optional<CartReservationExpiryReleaserModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CartReservationExpiryReleaserModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CartReservationExpiryReleaserModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CartReservationExpiryReleaserModel model = storageCache.get(mutationId);
        if (model != null) {
