...
/**
 * Enterprise Service Processor: EnvelopeEncryptionKmsClientService.
 * Encrypts Parquet column chunks using AWS KMS / HashiCorp Vault DEK envelope encryption.
 */
public class EnvelopeEncryptionKmsClientService {

    private static final Logger LOGGER = Logger.getLogger(EnvelopeEncryptionKmsClientService.class.getName());
    private final Map<String, EnvelopeEncryptionKmsClientModel> storageCache = new ConcurrentHashMap<>();
    ...
    public EnvelopeEncryptionKmsClientService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            EnvelopeEncryptionKmsClientModel model = new EnvelopeEncryptionKmsClientModel(
                mutId,
    ...
    public EnvelopeEncryptionKmsClientModel processIncomingMutation(EnvelopeEncryptionKmsClientModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in EnvelopeEncryptionKmsClientService");
    ...
    public Optional<EnvelopeEncryptionKmsClientModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<EnvelopeEncryptionKmsClientModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<EnvelopeEncryptionKmsClientModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        EnvelopeEncryptionKmsClientModel model = storageCache.get(mutationId);
        if (model != null) {
