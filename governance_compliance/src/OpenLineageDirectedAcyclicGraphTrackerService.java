package com.ecommerce.lakehouse.governance;
...
/**
 * Enterprise Service Processor: OpenLineageDirectedAcyclicGraphTrackerService.
 * Constructs end-to-end dataset and job lineage graphs for audit and impact analysis.
 */
public class OpenLineageDirectedAcyclicGraphTrackerService {

    private static final Logger LOGGER = Logger.getLogger(OpenLineageDirectedAcyclicGraphTrackerService.class.getName());
    private final Map<String, OpenLineageDirectedAcyclicGraphTrackerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public OpenLineageDirectedAcyclicGraphTrackerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            OpenLineageDirectedAcyclicGraphTrackerModel model = new OpenLineageDirectedAcyclicGraphTrackerModel(
                mutId,
    ...
    public OpenLineageDirectedAcyclicGraphTrackerModel processIncomingMutation(OpenLineageDirectedAcyclicGraphTrackerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in OpenLineageDirectedAcyclicGraphTrackerService");
    ...
    public Optional<OpenLineageDirectedAcyclicGraphTrackerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<OpenLineageDirectedAcyclicGraphTrackerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<OpenLineageDirectedAcyclicGraphTrackerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        OpenLineageDirectedAcyclicGraphTrackerModel model = storageCache.get(mutationId);
        if (model != null) {
