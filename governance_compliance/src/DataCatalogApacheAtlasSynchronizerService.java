...
/**
 * Enterprise Service Processor: DataCatalogApacheAtlasSynchronizerService.
 * Synchronizes table schemas, classifications, and glossary terms with Apache Atlas.
 */
public class DataCatalogApacheAtlasSynchronizerService {

    private static final Logger LOGGER = Logger.getLogger(DataCatalogApacheAtlasSynchronizerService.class.getName());
    private final Map<String, DataCatalogApacheAtlasSynchronizerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public DataCatalogApacheAtlasSynchronizerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            DataCatalogApacheAtlasSynchronizerModel model = new DataCatalogApacheAtlasSynchronizerModel(
                mutId,
    ...
    public DataCatalogApacheAtlasSynchronizerModel processIncomingMutation(DataCatalogApacheAtlasSynchronizerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in DataCatalogApacheAtlasSynchronizerService");
    ...
    public Optional<DataCatalogApacheAtlasSynchronizerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<DataCatalogApacheAtlasSynchronizerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<DataCatalogApacheAtlasSynchronizerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        DataCatalogApacheAtlasSynchronizerModel model = storageCache.get(mutationId);
        if (model != null) {
