...
/**
 * Enterprise Service Processor: AuditLogImmutableStorageManagerService.
 * Maintains immutable WORM storage of all administrative CDC configuration changes.
 */
public class AuditLogImmutableStorageManagerService {

    private static final Logger LOGGER = Logger.getLogger(AuditLogImmutableStorageManagerService.class.getName());
    private final Map<String, AuditLogImmutableStorageManagerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public AuditLogImmutableStorageManagerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            AuditLogImmutableStorageManagerModel model = new AuditLogImmutableStorageManagerModel(
                mutId,
    ...
    public AuditLogImmutableStorageManagerModel processIncomingMutation(AuditLogImmutableStorageManagerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in AuditLogImmutableStorageManagerService");
    ...
    public Optional<AuditLogImmutableStorageManagerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<AuditLogImmutableStorageManagerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<AuditLogImmutableStorageManagerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        AuditLogImmutableStorageManagerModel model = storageCache.get(mutationId);
        if (model != null) {
