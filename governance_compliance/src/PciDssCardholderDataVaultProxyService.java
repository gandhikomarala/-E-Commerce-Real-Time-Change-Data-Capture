...
/**
 * Enterprise Service Processor: PciDssCardholderDataVaultProxyService.
 * Isolates and encrypts PAN and CVV data in compliance with PCI-DSS 4.0 requirements.
 */
public class PciDssCardholderDataVaultProxyService {

    private static final Logger LOGGER = Logger.getLogger(PciDssCardholderDataVaultProxyService.class.getName());
    private final Map<String, PciDssCardholderDataVaultProxyModel> storageCache = new ConcurrentHashMap<>();
    ...
    public PciDssCardholderDataVaultProxyService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            PciDssCardholderDataVaultProxyModel model = new PciDssCardholderDataVaultProxyModel(
                mutId,
    ...
    public PciDssCardholderDataVaultProxyModel processIncomingMutation(PciDssCardholderDataVaultProxyModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in PciDssCardholderDataVaultProxyService");
    ...
    public Optional<PciDssCardholderDataVaultProxyModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<PciDssCardholderDataVaultProxyModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<PciDssCardholderDataVaultProxyModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        PciDssCardholderDataVaultProxyModel model = storageCache.get(mutationId);
        if (model != null) {
