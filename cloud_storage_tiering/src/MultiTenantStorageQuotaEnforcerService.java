...
/**
 * Enterprise Service Processor: MultiTenantStorageQuotaEnforcerService.
 * Enforces tenant-level storage byte quotas and throttles exceeding write rates.
 */
public class MultiTenantStorageQuotaEnforcerService {

    private static final Logger LOGGER = Logger.getLogger(MultiTenantStorageQuotaEnforcerService.class.getName());
    private final Map<String, MultiTenantStorageQuotaEnforcerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public MultiTenantStorageQuotaEnforcerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            MultiTenantStorageQuotaEnforcerModel model = new MultiTenantStorageQuotaEnforcerModel(
                mutId,
    ...
    public MultiTenantStorageQuotaEnforcerModel processIncomingMutation(MultiTenantStorageQuotaEnforcerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in MultiTenantStorageQuotaEnforcerService");
    ...
    public Optional<MultiTenantStorageQuotaEnforcerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<MultiTenantStorageQuotaEnforcerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<MultiTenantStorageQuotaEnforcerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        MultiTenantStorageQuotaEnforcerModel model = storageCache.get(mutationId);
        if (model != null) {
