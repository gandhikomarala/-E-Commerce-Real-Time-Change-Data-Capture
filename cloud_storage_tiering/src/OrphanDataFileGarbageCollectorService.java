...
/**
 * Enterprise Service Processor: OrphanDataFileGarbageCollectorService.
 * Identifies unreferenced Parquet data files across Iceberg snapshots.
 */
public class OrphanDataFileGarbageCollectorService {

    private static final Logger LOGGER = Logger.getLogger(OrphanDataFileGarbageCollectorService.class.getName());
    private final Map<String, OrphanDataFileGarbageCollectorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public OrphanDataFileGarbageCollectorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            OrphanDataFileGarbageCollectorModel model = new OrphanDataFileGarbageCollectorModel(
                mutId,
    ...
    public OrphanDataFileGarbageCollectorModel processIncomingMutation(OrphanDataFileGarbageCollectorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in OrphanDataFileGarbageCollectorService");
    ...
    public Optional<OrphanDataFileGarbageCollectorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<OrphanDataFileGarbageCollectorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<OrphanDataFileGarbageCollectorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        OrphanDataFileGarbageCollectorModel model = storageCache.get(mutationId);
        if (model != null) {
