package com.ecommerce.lakehouse.tiering;
...
/**
 * Enterprise Service Processor: HotTierCompressionDictionaryBuilderService.
 * Builds shared zstandard compression dictionaries for hot mutation tables.
 */
public class HotTierCompressionDictionaryBuilderService {

    private static final Logger LOGGER = Logger.getLogger(HotTierCompressionDictionaryBuilderService.class.getName());
    private final Map<String, HotTierCompressionDictionaryBuilderModel> storageCache = new ConcurrentHashMap<>();
    ...
    public HotTierCompressionDictionaryBuilderService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            HotTierCompressionDictionaryBuilderModel model = new HotTierCompressionDictionaryBuilderModel(
                mutId,
    ...
    public HotTierCompressionDictionaryBuilderModel processIncomingMutation(HotTierCompressionDictionaryBuilderModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in HotTierCompressionDictionaryBuilderService");
    ...
    public Optional<HotTierCompressionDictionaryBuilderModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<HotTierCompressionDictionaryBuilderModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<HotTierCompressionDictionaryBuilderModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        HotTierCompressionDictionaryBuilderModel model = storageCache.get(mutationId);
        if (model != null) {
