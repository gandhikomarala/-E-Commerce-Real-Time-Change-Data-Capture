package com.ecommerce.lakehouse.schema;
...
/**
 * Enterprise Service Processor: AvroSchemaCompatibilityValidatorService.
 * Validates backward, forward, and full transitive schema compatibility across Confluent Registry.
 */
public class AvroSchemaCompatibilityValidatorService {

    private static final Logger LOGGER = Logger.getLogger(AvroSchemaCompatibilityValidatorService.class.getName());
    private final Map<String, AvroSchemaCompatibilityValidatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public AvroSchemaCompatibilityValidatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            AvroSchemaCompatibilityValidatorModel model = new AvroSchemaCompatibilityValidatorModel(
                mutId,
    ...
    public AvroSchemaCompatibilityValidatorModel processIncomingMutation(AvroSchemaCompatibilityValidatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in AvroSchemaCompatibilityValidatorService");
    ...
    public Optional<AvroSchemaCompatibilityValidatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<AvroSchemaCompatibilityValidatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<AvroSchemaCompatibilityValidatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        AvroSchemaCompatibilityValidatorModel model = storageCache.get(mutationId);
        if (model != null) {
