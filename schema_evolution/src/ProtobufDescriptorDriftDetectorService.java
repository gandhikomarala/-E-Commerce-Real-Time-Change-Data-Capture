...
/**
 * Enterprise Service Processor: ProtobufDescriptorDriftDetectorService.
 * Detects gRPC protobuf schema field additions, enum value mutations, and deprecation flags.
 */
public class ProtobufDescriptorDriftDetectorService {

    private static final Logger LOGGER = Logger.getLogger(ProtobufDescriptorDriftDetectorService.class.getName());
    private final Map<String, ProtobufDescriptorDriftDetectorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ProtobufDescriptorDriftDetectorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ProtobufDescriptorDriftDetectorModel model = new ProtobufDescriptorDriftDetectorModel(
                mutId,
    ...
    public ProtobufDescriptorDriftDetectorModel processIncomingMutation(ProtobufDescriptorDriftDetectorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ProtobufDescriptorDriftDetectorService");
    ...
    public Optional<ProtobufDescriptorDriftDetectorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ProtobufDescriptorDriftDetectorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ProtobufDescriptorDriftDetectorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ProtobufDescriptorDriftDetectorModel model = storageCache.get(mutationId);
        if (model != null) {
