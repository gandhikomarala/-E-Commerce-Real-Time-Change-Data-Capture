...
/**
 * Enterprise Service Processor: ParquetTypeWideningCoercerService.
 * Safely coerces integer and float primitive types during schema widening migrations.
 */
public class ParquetTypeWideningCoercerService {

    private static final Logger LOGGER = Logger.getLogger(ParquetTypeWideningCoercerService.class.getName());
    private final Map<String, ParquetTypeWideningCoercerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ParquetTypeWideningCoercerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ParquetTypeWideningCoercerModel model = new ParquetTypeWideningCoercerModel(
                mutId,
    ...
    public ParquetTypeWideningCoercerModel processIncomingMutation(ParquetTypeWideningCoercerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ParquetTypeWideningCoercerService");
    ...
    public Optional<ParquetTypeWideningCoercerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ParquetTypeWideningCoercerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ParquetTypeWideningCoercerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ParquetTypeWideningCoercerModel model = storageCache.get(mutationId);
        if (model != null) {
