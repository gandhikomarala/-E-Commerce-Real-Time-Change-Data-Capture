...
/**
 * Enterprise Service Processor: GeofenceDeliveryArrivalNotifierService.
 * Dispatches automated SMS/push notifications when delivery driver crosses 500m geofence.
 */
public class GeofenceDeliveryArrivalNotifierService {

    private static final Logger LOGGER = Logger.getLogger(GeofenceDeliveryArrivalNotifierService.class.getName());
    private final Map<String, GeofenceDeliveryArrivalNotifierModel> storageCache = new ConcurrentHashMap<>();
    ...
    public GeofenceDeliveryArrivalNotifierService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            GeofenceDeliveryArrivalNotifierModel model = new GeofenceDeliveryArrivalNotifierModel(
                mutId,
    ...
    public GeofenceDeliveryArrivalNotifierModel processIncomingMutation(GeofenceDeliveryArrivalNotifierModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in GeofenceDeliveryArrivalNotifierService");
    ...
    public Optional<GeofenceDeliveryArrivalNotifierModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<GeofenceDeliveryArrivalNotifierModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<GeofenceDeliveryArrivalNotifierModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        GeofenceDeliveryArrivalNotifierModel model = storageCache.get(mutationId);
        if (model != null) {
