...
/**
 * Enterprise Service Processor: CarrierPerformanceScorecardAggregatorService.
 * Aggregates on-time delivery percentages, transit damage rates, and SLA breach chargebacks.
 */
public class CarrierPerformanceScorecardAggregatorService {

    private static final Logger LOGGER = Logger.getLogger(CarrierPerformanceScorecardAggregatorService.class.getName());
    private final Map<String, CarrierPerformanceScorecardAggregatorModel> storageCache = new ConcurrentHashMap<>();
    ...
    public CarrierPerformanceScorecardAggregatorService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            CarrierPerformanceScorecardAggregatorModel model = new CarrierPerformanceScorecardAggregatorModel(
                mutId,
    ...
    public CarrierPerformanceScorecardAggregatorModel processIncomingMutation(CarrierPerformanceScorecardAggregatorModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in CarrierPerformanceScorecardAggregatorService");
    ...
    public Optional<CarrierPerformanceScorecardAggregatorModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<CarrierPerformanceScorecardAggregatorModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<CarrierPerformanceScorecardAggregatorModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        CarrierPerformanceScorecardAggregatorModel model = storageCache.get(mutationId);
        if (model != null) {
