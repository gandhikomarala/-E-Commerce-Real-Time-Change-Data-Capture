...
/**
 * Enterprise Service Processor: LastMileDeliveryRoutePlannerService.
 * Optimizes vehicle routing problem (VRP) with time windows for last-mile delivery vans.
 */
public class LastMileDeliveryRoutePlannerService {

    private static final Logger LOGGER = Logger.getLogger(LastMileDeliveryRoutePlannerService.class.getName());
    private final Map<String, LastMileDeliveryRoutePlannerModel> storageCache = new ConcurrentHashMap<>();
    ...
    public LastMileDeliveryRoutePlannerService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            LastMileDeliveryRoutePlannerModel model = new LastMileDeliveryRoutePlannerModel(
                mutId,
    ...
    public LastMileDeliveryRoutePlannerModel processIncomingMutation(LastMileDeliveryRoutePlannerModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in LastMileDeliveryRoutePlannerService");
    ...
    public Optional<LastMileDeliveryRoutePlannerModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<LastMileDeliveryRoutePlannerModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<LastMileDeliveryRoutePlannerModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        LastMileDeliveryRoutePlannerModel model = storageCache.get(mutationId);
        if (model != null) {
