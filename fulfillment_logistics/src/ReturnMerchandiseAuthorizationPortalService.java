...
/**
 * Enterprise Service Processor: ReturnMerchandiseAuthorizationPortalService.
 * Generates automated prepaid return shipping labels and schedules carrier pickups.
 */
public class ReturnMerchandiseAuthorizationPortalService {

    private static final Logger LOGGER = Logger.getLogger(ReturnMerchandiseAuthorizationPortalService.class.getName());
    private final Map<String, ReturnMerchandiseAuthorizationPortalModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ReturnMerchandiseAuthorizationPortalService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ReturnMerchandiseAuthorizationPortalModel model = new ReturnMerchandiseAuthorizationPortalModel(
                mutId,
    ...
    public ReturnMerchandiseAuthorizationPortalModel processIncomingMutation(ReturnMerchandiseAuthorizationPortalModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ReturnMerchandiseAuthorizationPortalService");
    ...
    public Optional<ReturnMerchandiseAuthorizationPortalModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ReturnMerchandiseAuthorizationPortalModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ReturnMerchandiseAuthorizationPortalModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ReturnMerchandiseAuthorizationPortalModel model = storageCache.get(mutationId);
        if (model != null) {
