...
/**
 * Enterprise Service Processor: ClickHouseReplacingMergeTreeCdcBridgeService.
 * Streams CDC row mutations directly into ClickHouse ReplacingMergeTree tables.
 */
public class ClickHouseReplacingMergeTreeCdcBridgeService {

    private static final Logger LOGGER = Logger.getLogger(ClickHouseReplacingMergeTreeCdcBridgeService.class.getName());
    private final Map<String, ClickHouseReplacingMergeTreeCdcBridgeModel> storageCache = new ConcurrentHashMap<>();
    ...
    public ClickHouseReplacingMergeTreeCdcBridgeService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            ClickHouseReplacingMergeTreeCdcBridgeModel model = new ClickHouseReplacingMergeTreeCdcBridgeModel(
                mutId,
    ...
    public ClickHouseReplacingMergeTreeCdcBridgeModel processIncomingMutation(ClickHouseReplacingMergeTreeCdcBridgeModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in ClickHouseReplacingMergeTreeCdcBridgeService");
    ...
    public Optional<ClickHouseReplacingMergeTreeCdcBridgeModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<ClickHouseReplacingMergeTreeCdcBridgeModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<ClickHouseReplacingMergeTreeCdcBridgeModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        ClickHouseReplacingMergeTreeCdcBridgeModel model = storageCache.get(mutationId);
        if (model != null) {
