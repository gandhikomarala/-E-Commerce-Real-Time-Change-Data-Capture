...
/**
 * Enterprise Service Processor: StarRocksFastOlapMaterializedViewSyncService.
 * Synchronizes real-time StarRocks materialized view cubes over Iceberg tables.
 */
public class StarRocksFastOlapMaterializedViewSyncService {

    private static final Logger LOGGER = Logger.getLogger(StarRocksFastOlapMaterializedViewSyncService.class.getName());
    private final Map<String, StarRocksFastOlapMaterializedViewSyncModel> storageCache = new ConcurrentHashMap<>();
    ...
    public StarRocksFastOlapMaterializedViewSyncService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            StarRocksFastOlapMaterializedViewSyncModel model = new StarRocksFastOlapMaterializedViewSyncModel(
                mutId,
    ...
    public StarRocksFastOlapMaterializedViewSyncModel processIncomingMutation(StarRocksFastOlapMaterializedViewSyncModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in StarRocksFastOlapMaterializedViewSyncService");
    ...
    public Optional<StarRocksFastOlapMaterializedViewSyncModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<StarRocksFastOlapMaterializedViewSyncModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<StarRocksFastOlapMaterializedViewSyncModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        StarRocksFastOlapMaterializedViewSyncModel model = storageCache.get(mutationId);
        if (model != null) {
