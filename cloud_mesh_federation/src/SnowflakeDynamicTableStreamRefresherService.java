...
/**
 * Enterprise Service Processor: SnowflakeDynamicTableStreamRefresherService.
 * Refreshes Snowflake dynamic tables based on Iceberg metadata snapshot notifications.
 */
public class SnowflakeDynamicTableStreamRefresherService {

    private static final Logger LOGGER = Logger.getLogger(SnowflakeDynamicTableStreamRefresherService.class.getName());
    private final Map<String, SnowflakeDynamicTableStreamRefresherModel> storageCache = new ConcurrentHashMap<>();
    ...
    public SnowflakeDynamicTableStreamRefresherService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            SnowflakeDynamicTableStreamRefresherModel model = new SnowflakeDynamicTableStreamRefresherModel(
                mutId,
    ...
    public SnowflakeDynamicTableStreamRefresherModel processIncomingMutation(SnowflakeDynamicTableStreamRefresherModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in SnowflakeDynamicTableStreamRefresherService");
    ...
    public Optional<SnowflakeDynamicTableStreamRefresherModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<SnowflakeDynamicTableStreamRefresherModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<SnowflakeDynamicTableStreamRefresherModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        SnowflakeDynamicTableStreamRefresherModel model = storageCache.get(mutationId);
        if (model != null) {
