...
/**
 * Enterprise Service Processor: DatabricksUnityCatalogMetadataBridgeService.
 * Synchronizes table schemas and tags with Databricks Unity Catalog.
 */
public class DatabricksUnityCatalogMetadataBridgeService {

    private static final Logger LOGGER = Logger.getLogger(DatabricksUnityCatalogMetadataBridgeService.class.getName());
    private final Map<String, DatabricksUnityCatalogMetadataBridgeModel> storageCache = new ConcurrentHashMap<>();
    ...
    public DatabricksUnityCatalogMetadataBridgeService() {
        seedInitialCdcState();
    }
    ...
    private void seedInitialCdcState() {
        for (int i = 1; i <= 6; i++) {
            ...
            DatabricksUnityCatalogMetadataBridgeModel model = new DatabricksUnityCatalogMetadataBridgeModel(
                mutId,
    ...
    public DatabricksUnityCatalogMetadataBridgeModel processIncomingMutation(DatabricksUnityCatalogMetadataBridgeModel mutation) {
        long startNano = System.nanoTime();
        if (mutation == null || mutation.getMutationId() == null) {
            LOGGER.log(Level.WARNING, "Rejected null CDC mutation payload in DatabricksUnityCatalogMetadataBridgeService");
    ...
    public Optional<DatabricksUnityCatalogMetadataBridgeModel> findById(String mutationId) {
        if (mutationId == null) return Optional.empty();
        return Optional.ofNullable(storageCache.get(mutationId));
    }

    public List<DatabricksUnityCatalogMetadataBridgeModel> listAll() {
        return new ArrayList<>(storageCache.values());
    }

    public List<DatabricksUnityCatalogMetadataBridgeModel> listByMedallionLayer(String layer) {
    ...
    public boolean updateMedallionStage(String mutationId, String newLayer) {
        DatabricksUnityCatalogMetadataBridgeModel model = storageCache.get(mutationId);
        if (model != null) {
